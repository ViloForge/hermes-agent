"""ACP permission bridging for ViloForge dangerous-command approvals."""
...
# Maps ACP permission option ids to ViloForge approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match ViloForge approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping ViloForge semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into ViloForge approval strings."""
...
def make_approval_callback(
    ...
    """
    Return a ViloForge-compatible approval callback that bridges to ACP.
