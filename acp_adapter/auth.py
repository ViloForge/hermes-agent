"""ACP auth helpers — detect and advertise ViloForge authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active ViloForge runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if ViloForge can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for ViloForge.
    ...
    not have ViloForge provider credentials configured yet, so ViloForge always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate ViloForge using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure ViloForge provider",
            description=(
                "Open ViloForge' interactive model/provider setup in a terminal. "
                "Use this when ViloForge has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
