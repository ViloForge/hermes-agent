...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some ViloForge tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured ViloForge tool result clearly failed.
    ...
    # ViloForge core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # ViloForge read_file output is line-numbered with `|`. If we send it as raw
