"""ACP session manager — maps ACP sessions to ViloForge AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when ViloForge itself is running in WSL.
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch ViloForge from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ViloForge agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by ViloForge AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current ViloForge runtime provider configuration.
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the ViloForge daemon's process cwd.
