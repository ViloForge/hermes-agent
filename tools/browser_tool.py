...
def _build_browser_env() -> dict:
    ...
    Strips ViloForge-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when ViloForge silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the ViloForge CLI where sys.stdout and sys.stderr both report
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, ViloForge-managed
    ...
    # Build an extended search PATH including ViloForge-managed Node, macOS
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise ViloForge
