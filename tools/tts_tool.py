...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hermes/config.yaml``. ViloForge
...
# custom voice-cloning scripts, etc.) into ViloForge without any Python code
...
#           output_format: wav
#
# ViloForge writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when ViloForge actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where ViloForge caches Piper voice models.
