...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a ViloForge-managed provider credential (API key,
    ...
    Non-ViloForge API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are ViloForge-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a ViloForge-managed
    ...
    Non-ViloForge third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register ViloForge provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # ViloForge-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register ViloForge "
