#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the ViloForge Skills Hub.
...
# ---------------------------------------------------------------------------
# ViloForge centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("ViloForge index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("ViloForge index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized ViloForge Skills Index.
