...
def _get_active_profile_name() -> str:
    """Return the active ViloForge profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling ViloForge process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed ViloForge
    process leaves its container behind permanently because there's no
    subsequent ViloForge process scheduled to reuse that exact (task, profile)
    pair.
    """
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # _sanitize_label_value(); the active ViloForge profile is captured at
        ...
        # container shared across sessions").  If a prior ViloForge process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic ViloForge secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        # container) but can happen if a previous ViloForge process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every ViloForge exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        ViloForge process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next ViloForge startup. That covers the
        ...
        # outlives ViloForge processes, processes inside it stay alive, and
