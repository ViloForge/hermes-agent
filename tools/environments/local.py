...
# ViloForge-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# ViloForge-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a ViloForge inference secret exactly
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local ViloForge home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter ViloForge-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. ViloForge Agent requires Git for Windows on Windows.\n"
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, ViloForge trusts them.
    """
