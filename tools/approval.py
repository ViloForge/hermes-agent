...
DANGEROUS_PATTERNS = [
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/ViloForge file.
    ...
    # In-place edit of a ViloForge-managed security file (~/.hermes/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of ViloForge config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of ViloForge config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of ViloForge config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.hermes/ form so the ViloForge config/env patterns catch it. In Docker and
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute ViloForge home prefix to ``~/.hermes/``.
