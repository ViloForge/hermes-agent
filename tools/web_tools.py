...
When available, ViloForge can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via ViloForge config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through ViloForge' config/.env layer (``hermes config set``, ``hermes tools``)
