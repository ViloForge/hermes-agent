...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; ViloForge/<hermes_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; ViloForge/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    hermes_version = _get_hermes_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; ViloForge/{hermes_version})"
