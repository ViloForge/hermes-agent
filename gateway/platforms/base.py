...
# Audio file extensions ViloForge recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through ViloForge' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. ViloForge-created Telegram private-chat
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active ViloForge profile and shared ViloForge root both contain control
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and ViloForge
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ViloForge-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the ViloForge root (~/.hermes/.env,
...
class BasePlatformAdapter(ABC):
    ...
    # ViloForge commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: ViloForge sends
