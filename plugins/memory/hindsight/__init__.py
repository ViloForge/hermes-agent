...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so ViloForge can degrade gracefully instead of repeatedly trying to start
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this ViloForge config."""
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active ViloForge profile name (from agent_identity)
      {workspace} — ViloForge workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between ViloForge Agent and the User"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between ViloForge Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between ViloForge Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run ViloForge as a non-root user, or switch "
                ...
                # ViloForge get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When ViloForge created/used that client on the shared async loop,
