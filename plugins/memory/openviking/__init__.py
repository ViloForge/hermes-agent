...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — ViloForge peer ID in OpenViking (default: hermes)
...
_AGENT_PROMPT_LABEL = "ViloForge peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip ViloForge slash-skill scaffolding before sending content to OpenViking.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this ViloForge run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new ViloForge session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in ViloForge only", "write values only to ViloForge .env"),
    ...
    _print_openviking_ready("Connection saved to ViloForge .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "ViloForge peer ID in OpenViking, sent as the actor peer and "
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this ViloForge run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of ViloForge' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert ViloForge canonical messages into OpenViking batch payloads."""
