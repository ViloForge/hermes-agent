...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated ViloForge instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a ViloForge profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active ViloForge profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into ViloForge system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active ViloForge profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — ViloForge session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. ViloForge session title (from /title command; non-per-session)
