...
def cmd_sync(args) -> None:
    ...
    Scans all ViloForge profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current ViloForge profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives ViloForge persistent cross-session memory.")
    ...
    # These keys only affect the ViloForge GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the ViloForge gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
...
def _active_profile_name() -> str:
    """Return the active ViloForge profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    ViloForge' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. ViloForge asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → ViloForge + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → ViloForge\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives ViloForge persistent memory")
    ...
    if user_files:
        ...
        print("  (ViloForge calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In ViloForge, they are seeded once into Honcho's AI peer through the")
    ...
    print("    ViloForge:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    ViloForge:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    ViloForge:   Honcho observes every message and updates representations")
    ...
    print("    ViloForge:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to ViloForge Honcho",
    )
