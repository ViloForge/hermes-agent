"""langfuse — ViloForge plugin for Langfuse observability.

Traces ViloForge conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the ViloForge plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the ViloForge plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older ViloForge paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older ViloForge branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current ViloForge also has a turn-scoped
    ...
    # messages is a list only for legacy ViloForge branches that fired
    # pre_llm_call with API messages directly. Current ViloForge fires
...
def register(ctx) -> None:
    ...
    # ViloForge versions.  pre_api_request / post_api_request fire per API
