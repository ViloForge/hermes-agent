...
Optional ViloForge observability plugin that maps ViloForge observer hooks to
...
not replace ViloForge Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets ViloForge emit NeMo Relay lifecycle events for
the work ViloForge already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, ViloForge Agent can:

- Preserve ViloForge execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
`HERMES_HOME` is the ViloForge profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
ViloForge uses the user's default home, usually `~/.hermes`. For isolated smoke
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so ViloForge does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
ViloForge keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, ViloForge routes LLM and tool
...
For the full generic ViloForge middleware contract, see
...
This run starts a parent ViloForge session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='ViloForge Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "ViloForge Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. ViloForge dispatches the read-only tools as one batch, and NeMo Relay
...
- ViloForge sessions map to `agent` scopes.
- ViloForge API request hooks map to `llm` scope start/end events.
- ViloForge tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses ViloForge execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, ViloForge routes execution
through these boundaries:

```text
ViloForge provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> ViloForge provider adapter next_call(...)

ViloForge tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> ViloForge tool dispatcher next_call(...)
```
...
middleware for a local ViloForge run. This path requires a NeMo Relay runtime that
...
agent_name = "ViloForge Middleware E2E"
agent_version = "local"
