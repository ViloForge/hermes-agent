"""nemo_relay — optional ViloForge plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "ViloForge Agent"
...
class _Runtime:
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so ViloForge retry classification still sees it. The LLM and tool
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("HERMES_NEMO_RELAY_ATIF_AGENT_NAME") or "ViloForge Agent",
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # ViloForge wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and ViloForge
