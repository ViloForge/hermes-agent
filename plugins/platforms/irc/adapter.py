"""
IRC Platform Adapter for ViloForge Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the ViloForge agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :ViloForge Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :ViloForge Agent shutting down")
...
def interactive_setup() -> None:
    ...
    print_info("Connect ViloForge to an IRC network. Uses Python stdlib — no extra packages needed.")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :ViloForge Agent (cron)")
