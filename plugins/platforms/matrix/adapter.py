...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable ViloForge command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    ViloForge accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal ViloForge ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches ViloForge
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "ViloForge Agent" become "Agent".
        """
        if not body:
            return ""
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where ViloForge delivers cron job results and notifications.")
