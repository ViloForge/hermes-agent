// ViloForge Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> ViloForge): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (ViloForge -> gRPC): `/send` drives `space.send(...)`; `/typing`
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so ViloForge can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply ViloForge'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local ViloForge allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of ViloForge, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when ViloForge supplied them so a known-good
