...
This plugin connects ViloForge Agent to iMessage (and other Spectrum
...
TypeScript-only, ViloForge runs it inside a small supervised Node sidecar and
...
2. **Find or create** the `ViloForge Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other ViloForge channel,
