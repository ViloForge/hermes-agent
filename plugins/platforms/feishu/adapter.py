...
In single-bot mode (what ViloForge currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local ViloForge gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that ViloForge does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by ViloForge webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto ViloForge' SessionSource fields.
