...
ViloForge drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The ViloForge-side `computer_use` tool exposed
in this skill is a higher-level ViloForge vocabulary; the raw cua-driver
...
ViloForge intentionally keeps THIS skill focused on the ViloForge-side
...
When `cua-driver skills install` autodetects ViloForge (planned follow-up
