#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for ViloForge Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same ViloForge profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the ViloForge Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the ViloForge Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to ViloForge home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for ViloForge")
