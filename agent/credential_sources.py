"""Unified removal contract for every credential source ViloForge reads from.

ViloForge seeds its credential pool from many places:
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to ViloForge.",
            f"  The pool entry is now suppressed — ViloForge will ignore "
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so ViloForge stops reading it.
    """
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared ViloForge Anthropic OAuth credentials")
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the ViloForge auth store is not enough — _seed_from_singletons() would
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    ViloForge stops picking the token up.
    """
