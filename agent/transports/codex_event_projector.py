"""Projects codex app-server events into ViloForge' messages list.

The translator that lets ViloForge' memory/skill review keep working under the
...
ViloForge' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape ViloForge
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way ViloForge' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but ViloForge stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way ViloForge already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — ViloForge' messages store text only.
        text_parts: list[str] = []
