...
Owns one Codex thread per ViloForge session. Drives `turn/start`, consumes
...
# ViloForge' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse ViloForge/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per ViloForge session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but ViloForge' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into ViloForge' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved ViloForge' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map ViloForge approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    ViloForge returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort ViloForge version string for codex's userAgent line."""
    try:
