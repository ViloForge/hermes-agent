...
Plugins built on ViloForge Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing ViloForge
...
embedded slugs, no shorthands. This mirrors ViloForge' main config
...
handles every provider, fallback chain, and per-task override ViloForge
supports.
"""
