...
def _hermes_root_path() -> Path:
    """Resolve the ViloForge root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied ViloForge path.

    Three categories are blocked:

      * Internal ViloForge cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global ViloForge
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # ViloForge root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal ViloForge cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a ViloForge credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the ViloForge MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a ViloForge MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# ViloForge profiles are separate HERMES_HOME dirs under
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside ViloForge scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    ViloForge entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to ViloForge profile {info['target_profile']!r}, but the "
...
# regardless of which ViloForge profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner ViloForge-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative ViloForge state.
    ...
    Detection is path-shape-only — does not require any ViloForge resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host ViloForge process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative ViloForge state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host ViloForge "
