...
row taxonomy is inferred from the atlas shape so ViloForge can render both legacy
...
# rows (1536x1872), while older ViloForge/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are ViloForge' activity state names. They are not always identical to the
...
# Legacy ViloForge/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical ViloForge activity names -> accepted row-name aliases in descending
