...
ViloForge pulls API keys from Bitwarden Secrets Manager at process startup
...
* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  ViloForge pins one version (``_BWS_VERSION``) and downloads
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block ViloForge startup.  Missing binary, no network,
...
def _hermes_bin_dir() -> Path:
    """Where ViloForge stores its managed binaries.  Profile-aware."""
