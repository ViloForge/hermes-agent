...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # ViloForge-canonical
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a ViloForge wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical ViloForge
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ ViloForge is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical ViloForge-wire-shape response."""
