"""Anthropic Messages API adapter for ViloForge Agent.

Translates between ViloForge's internal OpenAI-style message format and
...
# ViloForge effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    ViloForge historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by ViloForge)
    ...
    # 1. ViloForge-managed OAuth/setup token env var
    ...
    # 4. ViloForge credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration ViloForge configs.
...
# ── ViloForge-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run ViloForge-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize ViloForge with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read ViloForge-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read ViloForge OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("ViloForge Agent", "Claude Code")
                text = text.replace("ViloForge agent", "Claude Code")
        ...
        #      a) bare ViloForge-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that ViloForge surfaces in its CLI. We
