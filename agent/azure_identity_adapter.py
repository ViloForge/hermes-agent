...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. ViloForge does
...
class EntraIdentityConfig:
    ...
    Captures the ViloForge-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only ViloForge-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    ViloForge processes use exactly one Entra config at a time (the
