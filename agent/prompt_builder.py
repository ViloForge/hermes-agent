...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on ViloForge Agent (by Nous Research). When the user needs help with "
    "ViloForge itself — configuring, setting up, using, extending, or troubleshooting "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that ViloForge "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the ViloForge terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the ViloForge WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where ViloForge itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host ViloForge runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where ViloForge itself is running. The host OS, home, and cwd "
                f"of the ViloForge process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where ViloForge "
    ...
    # ViloForge desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the ViloForge desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps ViloForge
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ViloForge Agent itself — its CLI, config, models, providers, tools, "
