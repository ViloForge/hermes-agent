...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with ViloForge Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working ViloForge setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [ViloForge Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="ViloForge Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up ViloForge for a team, bot, or always-on workflow
...
|---|---|---|
| I just want ViloForge working on my machine | `hermes setup` | Run a real chat and verify it responds |
...
**Rule of thumb:** if ViloForge cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install ViloForge Agent
### With the ViloForge Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the ViloForge Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.

### Without ViloForge Desktop:
For a command-line only install without ViloForge Desktop, run:
...
ViloForge separates secrets from normal config:
...
ViloForge ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hermes` vs `hermes --tui`.
:::
...
- ViloForge replies without error
...
```bash
# From the ViloForge install directory (the curl installer placed it at
...
/k8s                                       # load it and let ViloForge ask what you need
```
...
|---|---|---|
| ViloForge opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `hermes model` again and confirm provider, model, and auth |
