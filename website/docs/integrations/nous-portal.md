...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run ViloForge Agent"
---
...
| **ViloForge** | Hermes-4-70B, Hermes-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes ViloForge Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a ViloForge-side opinion — it's the official guidance from Nous Research.
...
If you already have ViloForge configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
OAuth needs a browser, but the loopback callback runs on the machine where ViloForge is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding, `--manual-paste` for browser-only environments like Cloud Shell / Codespaces).

### Profile setup

If you use [ViloForge profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
ViloForge mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so ViloForge stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `hermes auth add nous` to log in again; the quarantine clears on the next successful login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/hermes-agent/issues) — we surface the Portal's catalog to ViloForge and gaps usually mean a routing config we can update.
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-ViloForge tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple ViloForge configurations sharing one Portal login
