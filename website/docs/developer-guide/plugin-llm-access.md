...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept ViloForge already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — ViloForge provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different ViloForge provider
        # (must be one ViloForge already knows about — same names as
        ...
        # plugin sends — ViloForge does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through ViloForge' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing ViloForge subsystem:
