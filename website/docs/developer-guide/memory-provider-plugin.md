...
description: "How to build a memory provider plugin for ViloForge Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give ViloForge Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
the `messages` parameter; ViloForge will continue calling them with the legacy
signature.
