...
description: "How ViloForge builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

ViloForge deliberately separates:
...
You are ViloForge, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance ViloForge
...
```
You are ViloForge Agent, an intelligent AI assistant created by Nous Research.
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | ViloForge-native project config |
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, ViloForge concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs ViloForge already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking ViloForge.
...
- if you want to change how ViloForge assembles prompts for everyone, change Python and treat it as a code contribution
