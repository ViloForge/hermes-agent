...
description: "How to add a new inference provider to ViloForge Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

ViloForge can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:
...
- What env vars should ViloForge check, and in what priority order?
...
Be careful with API-key precedence. ViloForge already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.
...
When you add a native provider, double-check that ViloForge is only sending fields that provider actually understands.
