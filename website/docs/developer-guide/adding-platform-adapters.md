...
This guide covers adding a new messaging platform to the ViloForge gateway. A platform adapter connects ViloForge to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core ViloForge code. Your plugin is a directory with two files:
...
version: 1.0.0
description: >
  My Platform gateway adapter for ViloForge Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the ViloForge core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
