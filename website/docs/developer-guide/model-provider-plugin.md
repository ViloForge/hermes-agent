...
description: "How to build a model provider (inference backend) plugin for ViloForge Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that ViloForge can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$HERMES_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with ViloForge
...
Four values are recognized. ViloForge picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but ViloForge' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Like any ViloForge plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:
...
See [Building a ViloForge Plugin](/guides/build-a-hermes-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a ViloForge Plugin](/guides/build-a-hermes-plugin) — general plugin authoring
