...
description: "How ViloForge detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in ViloForge' browser tooling:
...
One `asyncio.Task` running in a background daemon thread per ViloForge `task_id`.
