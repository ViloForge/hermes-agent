...
description: "How to build a web-search/extract/crawl backend plugin for ViloForge Agent"
---
...
:::tip
Web search is one of several **backend plugins** ViloForge supports. The others (with their own ABCs) are [Image Generation Provider Plugins](/developer-guide/image-gen-provider-plugin), [Video Generation Provider Plugins](/developer-guide/video-gen-provider-plugin), [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a ViloForge Plugin](/guides/build-a-hermes-plugin).
:::

## How discovery works

ViloForge scans for web-search backends in three places:
...
When neither key is set, ViloForge auto-detects the backend from whichever API key/URL is present in the environment. `hermes tools` walks users through selection.
...
| `requires_env` | Interactive credential prompt during `hermes plugins install` (see [Build a ViloForge Plugin](/guides/build-a-hermes-plugin#gate-on-environment-variables) for the rich format) |
...
ViloForge routes calls to the right provider based on the `supports_*` flags. A common multi-provider setup:
...
When `web.search_backend` or `web.extract_backend` aren't set, both fall through to `web.backend`. When that's also unset, ViloForge picks the first available provider that supports the requested capability based on env-var presence.
...
## How ViloForge wires it into the tools
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — ViloForge will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a ViloForge Plugin → Lazy-install](/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a ViloForge Plugin](/guides/build-a-hermes-plugin) — general tools/hooks/slash commands guide
