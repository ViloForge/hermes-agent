...
2. ViloForge stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, ViloForge calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default ViloForge **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every ViloForge process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "ViloForge keys").
...
4. **Machine accounts → New machine account → My ViloForge machine** → **Projects** tab → grant Read access to your project.
...
Bitwarden never blocks ViloForge startup. If anything goes wrong, you'll see a one-line warning in stderr and ViloForge continues with whatever credentials `.env` already had:
...
- ViloForge will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — ViloForge does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple ViloForge installations.
