...
ViloForge Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `hermes tools` and persists in `config.yaml`.
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; ViloForge materializes them to the local cache so delivery still works after expiry
