...
description: "Overview of ViloForge Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
ViloForge ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
**One persistent container, shared across the whole process.** ViloForge starts a single long-lived container on first use (`docker run -d ... sleep 2h`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the ViloForge process. The container is stopped and removed on shutdown.
