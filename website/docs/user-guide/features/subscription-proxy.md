...
chat completions — use your ViloForge-managed provider subscription as their
...
| Use case | "Use ViloForge as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. ViloForge stores
the refresh token in `~/.hermes/auth.json` — the same place all ViloForge
