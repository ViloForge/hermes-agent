...
ViloForge has three hook systems that run custom code at key lifecycle points:
...
This tutorial shows how to build it yourself as a user-defined hook. ViloForge does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.
...
An earlier version of ViloForge shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is ViloForge's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
def notify_approval(command, description, session_key, **kwargs):
    title = "ViloForge needs approval"
...
Declare shell-script hooks in your `cli-config.yaml` and ViloForge will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Each time the event fires, ViloForge spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // ViloForge-canonical
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate ViloForge event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time ViloForge sees it, then persists the decision to `~/.hermes/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
