...
description: "Connect ViloForge Agent to external tool servers via MCP — and control exactly which MCP tools ViloForge loads"
---

# MCP (Model Context Protocol)

MCP lets ViloForge Agent connect to external tool servers so the agent can use tools that live outside ViloForge itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted ViloForge to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native ViloForge tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want ViloForge to see
...
4. Ask ViloForge to use the MCP-backed capability.
...
ViloForge will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

ViloForge ships a curated catalog of MCP servers that Nous staff has reviewed
...
```
n8n          available              Manage and inspect n8n workflows from ViloForge
...
- **API key** — ViloForge prompts at install time and writes the value to
...
- **OAuth** (third-party provider like Google/GitHub) — ViloForge points you at
  `hermes auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, ViloForge probes the MCP server to list every
...
PR adds an entry with a newer `manifest_version` than your installed ViloForge
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
ViloForge`) for that entry instead of silently hiding it. Run `hermes update`
to install the latest ViloForge when you see that.
...
after a ViloForge update if a manifest version changed.
...
HTTP MCP servers are remote endpoints ViloForge connects to directly.
...
- you do not want ViloForge spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and ViloForge handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, ViloForge prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.hermes/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When ViloForge runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal ViloForge prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
...
Then run `hermes mcp login googledrive` — with the pre-registered client, ViloForge skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.hermes/config.yaml` from inside a running ViloForge session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `hermes mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. ViloForge passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
ViloForge reads MCP config from `~/.hermes/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, ViloForge skips the server entirely |
...
## How ViloForge registers MCP tools

ViloForge prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — ViloForge sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, ViloForge also registers utility tools around MCP resources and prompts:
...
- ViloForge only registers resource utilities if the MCP session actually supports resource operations
- ViloForge only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to ViloForge, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, ViloForge skips the server completely and does not even attempt a connection.
...
You can also separately disable ViloForge-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, ViloForge does not create an empty runtime MCP toolset for that server.
...
ViloForge discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify ViloForge when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When ViloForge receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, ViloForge does not blindly pass your full shell environment.
...
Then verify your config and restart ViloForge.
...
Because ViloForge now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, ViloForge may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from ViloForge via the `sampling/createMessage` protocol. This allows an MCP server to ask ViloForge to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running ViloForge as an MCP server

In addition to connecting **to** MCP servers, ViloForge can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use ViloForge's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through ViloForge
- You want a single MCP server that bridges to all of ViloForge's connected messaging platforms at once
- You already have a running ViloForge gateway with connected platforms
...
Add ViloForge to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed ViloForge in a specific location:
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a ViloForge-specific channel browser:
...
The MCP server includes a live event bridge that polls ViloForge's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
The MCP server reads conversation data directly from ViloForge's session store (`~/.hermes/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the ViloForge agent itself.
...
- The embedded `hermes mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of ViloForge, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
...
- [Use MCP with ViloForge](/guides/use-mcp-with-hermes)
