...
ViloForge Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
When a memory provider is active, ViloForge automatically:
...
Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per ViloForge profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All ViloForge profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per ViloForge profile. Host key `hermes` → default; `hermes.<profile>` for others. |
...
Scans every ViloForge profile, creates host blocks for any profile without one, inherits settings from the default `hermes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
`OPENVIKING_AGENT` is ViloForge' peer ID in OpenViking for peer-scoped memories.
...
| `retain_context` | `conversation between ViloForge Agent and the User` | Context label for retained memories |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per ViloForge profile
