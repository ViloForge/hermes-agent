...
**Tool Search** is ViloForge' opt-in progressive-disclosure layer for that
...
:::info Built-in ViloForge tools never defer
The tools that make up ViloForge' core capability set (`terminal`,
...
When the model invokes `tool_call`, ViloForge **unwraps the bridge** and
...
- **No JS sandbox.** ViloForge uses the simpler "structured tools" mode
