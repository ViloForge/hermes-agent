...
Schedule tasks to run automatically with natural language or cron expressions. ViloForge exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to ViloForge itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. ViloForge disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
Ask ViloForge normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

ViloForge will use the unified `cronjob` tool internally.
...
The `cronjob` tool's schema exposes `no_agent` to ViloForge directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

ViloForge will write the check script to `~/.hermes/scripts/` via `write_file`, then call:
...
- When Job 2 fires, ViloForge reads Job 1's most recent output from `~/.hermes/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, ViloForge skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `hermes tools` cron-platform config wins; otherwise ViloForge falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether ViloForge should even invoke the agent. Emit a final stdout line of the form:
...
:::tip
ViloForge's own `~/.hermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, ViloForge resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
