...
ViloForge Agent includes two model-callable web tools backed by multiple providers:
...
Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Tavily/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let ViloForge lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).
...
The summary keeps quotes, code blocks, and key facts in their original formatting — it's a content compressor, not a paraphraser. If summarization fails or times out, ViloForge falls back to the first ~5 000 chars of raw content rather than a useless error.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point ViloForge at a running SearXNG instance.
...
# After (enable JSON for ViloForge):
formats:
  - html
  - json
```
...
With this config, ViloForge uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.
...
If no backend is explicitly configured, ViloForge picks the first available one based on which credentials are set:
