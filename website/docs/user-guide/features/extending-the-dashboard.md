...
description: "Build themes and plugins for the ViloForge web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The ViloForge web dashboard (`hermes dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **ViloForge Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **ViloForge Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except ViloForge Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other ViloForge plugins in `~/.hermes/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
// ViloForge API client + raw fetcher
...
#### Calling built-in ViloForge endpoints
...
| `header-left` | Before the ViloForge brand in the top bar. |
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **ViloForge Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
1. Confirm the plugin is bundled with ViloForge. User-installed and project dashboard plugins can extend the UI, but their Python backend routes are not auto-imported.
