...
title: "Plugins"
description: "Extend ViloForge with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

ViloForge has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in ViloForge
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a ViloForge Plugin](/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.
...
Start ViloForge — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal ViloForge plugin — registers a tool and a hook."""
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart ViloForge, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting ViloForge.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with ViloForge — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
Within each source, ViloForge also recognizes sub-category directories that route plugins to specialized discovery systems:
...
Several categories of plugin bypass `plugins.enabled` — they're part of ViloForge' built-in surface and would break basic functionality if gated off by default:
...
ViloForge has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and ViloForge also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a ViloForge Plugin](/guides/build-a-hermes-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a ViloForge Plugin](/guides/build-a-hermes-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a ViloForge Plugin](/guides/build-a-hermes-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. ViloForge auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
