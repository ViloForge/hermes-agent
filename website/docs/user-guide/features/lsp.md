...
ViloForge runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. ViloForge
...
1. ViloForge captures a baseline of current diagnostics for the file.
...
…). ViloForge auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — ViloForge installs both packages together when you
...
When `install_strategy: auto`, ViloForge installs binaries into
...
shared location — the staging dir is fully ViloForge-owned and is
...
Servers are kept alive for the life of the ViloForge process. There's
