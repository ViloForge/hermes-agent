...
description: "Customize ViloForge Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

ViloForge Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who ViloForge is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

ViloForge now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run ViloForge with a custom home directory, it will use:
...
- ViloForge creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- ViloForge loads `SOUL.md` only from `HERMES_HOME`
- ViloForge does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, ViloForge falls back to a built-in default identity
...
If ViloForge loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the ViloForge instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change ViloForge' default personality."
...
- how ViloForge should handle uncertainty, disagreement, or ambiguity
...
## What ViloForge injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, ViloForge falls back to a built-in default identity ("You are ViloForge Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
ViloForge ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain ViloForge, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives ViloForge its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how ViloForge speaks
- `display.skin` and `/skin` affect how ViloForge looks in the terminal
