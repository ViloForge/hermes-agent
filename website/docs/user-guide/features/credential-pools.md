...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, ViloForge automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, ViloForge auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:
...
ViloForge automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| ViloForge PKCE OAuth | `~/.hermes/auth.json` | Yes (Anthropic) |
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. ViloForge can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and ViloForge-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to ViloForge' credential store, so its token remains persistable.
