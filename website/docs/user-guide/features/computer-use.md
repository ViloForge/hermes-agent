...
ViloForge Agent can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or ViloForge app). `hermes computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each ViloForge run declares its own cua-driver
...
ViloForge intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the ViloForge-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the ViloForge skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **ViloForge
...
ViloForge applies multi-layer guardrails:
...
Screenshots are expensive. ViloForge applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  ViloForge agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the ViloForge agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. ViloForge warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive ViloForge from inside the RDP / console
...
upstream. **ViloForge disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) ViloForge sets
...
unreleased fix — point ViloForge at a binary you built from source instead
of the published release. ViloForge resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for ViloForge testing (see notes).
...
### Option B — point ViloForge straight at the built binary (fastest loop)
...
### Confirm ViloForge is using your build
...
- **ViloForge spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no ViloForge
  change needed either way.
- **Local builds skip the version check.** ViloForge warns when the
...
- **ViloForge-side skill** — `skills/computer-use/SKILL.md` — teaches the
  ViloForge `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects ViloForge (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (ViloForge among them)
