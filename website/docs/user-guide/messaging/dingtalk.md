...
sidebar_position: 10
title: "DingTalk"
description: "Set up ViloForge Agent as a DingTalk chatbot"
---

# DingTalk Setup

ViloForge Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how ViloForge behaves once it's in your DingTalk workspace.

## How ViloForge Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | ViloForge responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | ViloForge responds when you `@mention` it. Without a mention, ViloForge ignores the message. |
| **Shared groups with multiple users** | By default, ViloForge isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
ViloForge Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
ViloForge can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
ViloForge automatically adds emoji reactions to your messages to show processing status:
