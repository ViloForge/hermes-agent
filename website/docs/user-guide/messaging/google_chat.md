...
sidebar_position: 12
title: "Google Chat"
description: "Set up ViloForge Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect ViloForge Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your ViloForge
...
download the file. Save it somewhere only ViloForge can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — ViloForge calls
...
- **App name**: whatever you want users to see ("ViloForge" is reasonable).
...
`ADDED_TO_SPACE` event that ViloForge uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure ViloForge
...
Install the dependencies the Google Chat adapter needs (no ViloForge extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "ViloForge is thinking…" marker, then
...
Thread support: when a user replies inside a thread, ViloForge detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate ViloForge session.
...
3. Download the JSON. Move it onto the host that runs ViloForge.
4. Register the client with ViloForge (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's ViloForge home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, ViloForge isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: ViloForge will only attach the SA bearer
