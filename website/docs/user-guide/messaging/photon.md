...
Connect ViloForge to **iMessage** through [Photon][photon], a managed
...
your first iMessage from ViloForge — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, ViloForge runs it in a
...
2. **Finds or creates** the `ViloForge Agent` project on your account.
...
Photon uses the same authorization model as every other ViloForge
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, ViloForge replies with a pairing code. Approve it with:
...
By default ViloForge responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `ViloForge` and
`@ViloForge agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and ViloForge will reply.
...
assigned iMessage line ViloForge uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** ViloForge sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | ViloForge wake words  | JSON list / comma / newline regex patterns for group mentions |
