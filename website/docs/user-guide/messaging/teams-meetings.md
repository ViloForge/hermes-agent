...
Use the Teams meeting pipeline when you want ViloForge to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Use this when you want ViloForge to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start ViloForge normally after updating config:

```bash
hermes gateway run
```

Or, if you run ViloForge in Docker, start the gateway the same way you already do for your deployment.
