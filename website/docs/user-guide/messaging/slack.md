...
sidebar_position: 4
title: "Slack"
description: "Set up ViloForge Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect ViloForge Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your ViloForge instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. ViloForge uses the modern
...
The fastest path is to paste a manifest ViloForge generates for you. It
...
### Option A: From a ViloForge-generated manifest (recommended)
...
4. Enter an app name (e.g., "ViloForge Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, ViloForge can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a ViloForge configuration issue.
:::
...
ViloForge uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every ViloForge command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
ViloForge command with its description.

Under the hood: ViloForge ships with a generated Slack app manifest (see
...
When ViloForge adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — ViloForge routes it the same way as `/btw
...
Slack never delivers them to ViloForge.

As a workaround, ViloForge recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
ViloForge treats it identically to the slash form and replies in the same
thread.
...
ViloForge falls back to a text prompt, the prompt instructs you to reply
...
Understanding how ViloForge behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@ViloForge Agent what time is it?`). In channels, ViloForge replies in a thread attached to that message. |
| **Threads** | If you @mention ViloForge inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to ViloForge in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before ViloForge will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where ViloForge will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @ViloForge Agent`).

---

## Multi-Workspace Support

ViloForge can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, ViloForge also loads tokens from an **OAuth token file** at:
...
- When a message arrives, ViloForge uses the correct workspace-specific client to respond.
...
ViloForge supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @ViloForge Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @ViloForge Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. ViloForge now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @ViloForge Agent`)
...
- Audit who has access to your ViloForge config directory
