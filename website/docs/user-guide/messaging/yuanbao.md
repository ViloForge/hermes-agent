...
sidebar_position: 16
title: "Yuanbao"
description: "Connect ViloForge Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect ViloForge to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
All standard ViloForge commands work on Yuanbao:
...
Yuanbao has a maximum message size. ViloForge automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
