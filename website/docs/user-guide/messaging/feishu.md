...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up ViloForge Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

ViloForge Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; ViloForge opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | ViloForge responds to every message. |
| Group chats | ViloForge responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. ViloForge will automatically create a bot application with the correct permissions and save the credentials.
...
Use WebSocket mode when ViloForge runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run ViloForge behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, ViloForge starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how ViloForge responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | ViloForge responds to @mentions from any user in any group. |
| `allowlist` | ViloForge only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | ViloForge ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let ViloForge read all group traffic without requiring an @mention:
...
ViloForge auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default ViloForge ignores messages sent by other bots. Enable bot-to-bot messaging when you want ViloForge to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions ViloForge. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `hermes update --gateway` needs confirmation, the adapter records the selected answer in ViloForge's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, ViloForge reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
You can invite the ViloForge Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, ViloForge can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends ViloForge the meeting invitation event.
- ViloForge extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, ViloForge drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so ViloForge can reply to the inviter.
...
| `Another local ViloForge gateway is already using this Feishu app_id` | Only one ViloForge instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | ViloForge can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
