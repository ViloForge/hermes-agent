...
title: "SMS (Twilio)"
description: "Set up ViloForge Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

ViloForge connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
If you're running ViloForge locally, use a tunnel to expose the webhook:
...
Text your Twilio number — ViloForge will respond via SMS.
...
ViloForge validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check ViloForge gateway logs for Twilio API errors
