...
sidebar_position: 9
title: "Matrix"
description: "Set up ViloForge Agent as a Matrix bot"
---

# Matrix Setup

ViloForge Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the ViloForge Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

ViloForge works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how ViloForge behaves once it's connected.

## How ViloForge Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | ViloForge responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, ViloForge requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | ViloForge supports Matrix threads (MSC3440). If you reply in a thread, ViloForge keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, ViloForge auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | ViloForge accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; ViloForge normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, ViloForge isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
ViloForge sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
ViloForge now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run ViloForge as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

ViloForge needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give ViloForge the bot's user ID and password. ViloForge will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
ViloForge Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure ViloForge Agent
...
Bridge and appservice deployments need extra loop protection. ViloForge always
...
ViloForge supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
When E2EE is enabled, ViloForge:
...
In Matrix conversations, ViloForge exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when ViloForge knows who requested the action.

### Media Limits

ViloForge uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. ViloForge rejects arbitrary
...
ViloForge includes an opt-in Synapse harness for local validation:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, ViloForge imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If ViloForge bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
ViloForge detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart ViloForge:
   ```bash
...
ViloForge supports the same gateway commands in Matrix that it supports on other
...
ViloForge only normalizes `!command` when the command is known to the gateway, a
...
If you previously used ViloForge with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "ViloForge Agent"
     }'
   ```
...
a working E2EE setup with a previous version of ViloForge and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote ViloForge API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: ViloForge uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps ViloForge's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the ViloForge logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The ViloForge gateway isn't running, or it failed to connect.
...
For more information on securing your ViloForge Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: ViloForge can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
