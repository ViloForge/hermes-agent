...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to ViloForge Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With ViloForge Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to ViloForge Agent's API server just like it would connect to OpenAI. ViloForge handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **ViloForge agent runtime**, not a pure LLM proxy. For each request, ViloForge creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at a ViloForge API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to ViloForge server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.

## Quick Setup

### One-command local bootstrap (macOS/Linux, no Docker)

If you want ViloForge + Open WebUI wired together locally with a reusable launcher, run:
...
- restarts the ViloForge gateway so the API server comes up
...
- ViloForge API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `ViloForge Agent`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My ViloForge UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
HERMES_API_MODEL_NAME='My ViloForge Agent' \
bash scripts/setup_open_webui.sh
```
...
### 2. Start ViloForge Agent gateway
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in ViloForge
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and ViloForge Agent responds accordingly. Each request includes the full conversation history.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and ViloForge Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, ViloForge also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. ViloForge Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server ViloForge instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run ViloForge locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://github.com/NousResearch/hermes-agent/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | ViloForge Agent's API URL (include `/v1`) |
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your ViloForge models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
ViloForge Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in ViloForge Agent.
...
To run separate ViloForge instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated ViloForge agent.
