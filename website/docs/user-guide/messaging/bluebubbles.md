...
Connect ViloForge to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between ViloForge and the BlueBubbles server
...
By default, ViloForge responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, ViloForge uses conservative defaults for `ViloForge` and `@ViloForge agent` variants.
...
When someone messages your iMessage, ViloForge automatically sends them a pairing code. Approve it with:
```bash
...
ViloForge will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → ViloForge
ViloForge → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** ViloForge sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | ViloForge wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |
...
### Chat Addressing
You can address chats by email or phone number — ViloForge resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
