...
sidebar_position: 8
title: "Mattermost"
description: "Set up ViloForge Agent as a Mattermost bot"
---

# Mattermost Setup

ViloForge Agent integrates with Mattermost as a bot, letting you chat with your AI assistant through direct messages or team channels. Mattermost is a self-hosted, open-source Slack alternative — you run it on your own infrastructure, keeping full control of your data. The bot connects via Mattermost's REST API (v4) and WebSocket for real-time events, processes messages through the ViloForge Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, and slash commands.

No external Mattermost library is required — the adapter uses `aiohttp`, which is already a ViloForge dependency.

Before setup, here's the part most people want to know: how ViloForge behaves once it's in your Mattermost instance.
...
| **Public/private channels** | ViloForge responds when you `@mention` it. Without a mention, ViloForge ignores the message. |
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, ViloForge replies in a thread under your message. Thread context stays isolated from the parent channel. |
| **Shared channels with multiple users** | By default, ViloForge isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |

:::tip
If you want ViloForge to reply as threaded conversations (nested under your original message), set `MATTERMOST_REPLY_MODE=thread`. The default is `off`, which sends flat messages in the channel.
:::
...
3. Fill in the details:
   - **Username**: e.g., `hermes`
   - **Display Name**: e.g., `ViloForge Agent`
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Profile** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want ViloForge to post as your own user rather than a separate bot user.
:::
...
ViloForge Agent uses your Mattermost User ID to control who can interact with the bot. To find it:
...
## Step 5: Configure ViloForge Agent
...
The `MATTERMOST_REPLY_MODE` setting controls how ViloForge posts responses:

| Mode | Behavior |
|------|----------|
| `off` (default) | ViloForge posts flat messages in the channel, like a normal user. |
| `thread` | ViloForge replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth. |
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with ViloForge Agent.
