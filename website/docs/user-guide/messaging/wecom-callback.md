...
Connect ViloForge to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
ViloForge supports two WeCom integration modes:
...
3. ViloForge decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in ViloForge and the token the
