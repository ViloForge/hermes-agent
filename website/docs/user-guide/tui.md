...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for ViloForge — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for ViloForge — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run ViloForge interactively.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while ViloForge is starting.
...
On first launch ViloForge installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new ViloForge version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point ViloForge at it:
...
<img alt="ViloForge TUI Session Orchestrator with one live session and a +new row" src="/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="ViloForge TUI Session Orchestrator demo" />
...
The status-bar busy indicator is pluggable — the default rotates ViloForge' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
The TUI respects all standard ViloForge config: `~/.hermes/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
If the TUI fails to launch (no Node, missing bundle, TTY issue), ViloForge prints a diagnostic and falls back — rather than leaving you stuck.
