...
ViloForge Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
ViloForge stores session history so it can resume conversations, but it does not
...
ViloForge explicitly injects for that turn.
...
For example, if a user sends an image and asks ViloForge to make a meme from it,
ViloForge may inspect that image once with vision and run an image-processing
...
When you resume a session, ViloForge displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ViloForge session." />
...
When a session's context is compressed (manually via `/compress` or automatically), ViloForge creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:
...
ViloForge provides a full set of session management commands via `hermes sessions`:
...
When ViloForge cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ViloForge uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ViloForge in the same Discord channel without sharing transcript history
...
read by ViloForge. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every ViloForge process in the same `HERMES_HOME`
