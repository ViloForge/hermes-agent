...
files. It is **never persisted** to any `.env` by ViloForge.
...
If you try to change a managed value, ViloForge refuses and names the source:
...
Changes take effect on the next ViloForge start (a malformed managed file is logged
...
- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs ViloForge as `root`), managed scope is advisory.
