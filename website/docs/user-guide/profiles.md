...
Run multiple independent ViloForge agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate ViloForge home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up ViloForge state.
...
That's it. `coder` is now its own ViloForge profile with its own config, memory, and state.
...
- A **profile** gives ViloForge its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory ViloForge was launched from", not "the profile directory".
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.hermes/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, ViloForge state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls ViloForge config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other ViloForge
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, ViloForge keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode ViloForge launches tool
...
ViloForge also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
