...
title: "Git Worktrees"
description: "Run multiple ViloForge agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ViloForge Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ViloForge so each session has a clean, isolated working directory.

## Why Use Worktrees with ViloForge?

ViloForge treats the **current working directory** as the project root:
...
Now you can `cd` into the new worktree and run ViloForge there:

```bash
cd ../repo-feature

# Start ViloForge in the worktree
hermes
```
...
- ViloForge checkpoint data under `~/.hermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ViloForge experiment**
...
- **Avoid running ViloForge from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hermes -w` (Automatic Worktree Mode)

ViloForge has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Use **git worktrees** to give each ViloForge session its own clean checkout.
