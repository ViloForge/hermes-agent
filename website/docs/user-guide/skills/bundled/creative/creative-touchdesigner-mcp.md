...
```
ViloForge Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to ViloForge config (if missing)
...
3. **Restart ViloForge session** to pick up the new MCP server
