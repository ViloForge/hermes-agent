...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the ViloForge terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For ViloForge itself, `model.provider: openai-codex` uses ViloForge-managed Codex
...
## ViloForge Gateway Caveat

When invoking the Codex CLI from a ViloForge gateway/service context (for example,
