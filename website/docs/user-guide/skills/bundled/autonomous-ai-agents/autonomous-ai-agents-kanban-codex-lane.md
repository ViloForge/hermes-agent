...
description: "Use when a ViloForge Kanban worker wants to run Codex CLI as an isolated implementation lane while ViloForge keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a ViloForge Kanban worker wants to run Codex CLI as an isolated implementation lane while ViloForge keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
:::info
The following is the complete skill definition that ViloForge loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.
:::
...
This skill defines the lightweight ViloForge+Codex dual-lane convention for Kanban workers. ViloForge is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a ViloForge worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn ViloForge workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by ViloForge in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- ViloForge can run the relevant tests itself after Codex exits.
...
1. ViloForge owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any ViloForge board CLI as a substitute for the worker.
2. ViloForge owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. ViloForge owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from ViloForge with the repo's canonical wrapper.
4. ViloForge owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. ViloForge owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional ViloForge edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if ViloForge can do the task directly.
...
ViloForge owns the Kanban lifecycle; do not call ViloForge kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on ViloForge reconciliation.
...
- Explicit statement: ViloForge owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands ViloForge will run afterward.
...
ViloForge must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by ViloForge.
...
- [ ] ViloForge ran the canonical tests itself, using `scripts/run_tests.sh` for ViloForge Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from ViloForge-run tests.
- [ ] Accepted commits/diffs were applied to the ViloForge-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct ViloForge edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from ViloForge.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but ViloForge writes board state.
...
- [ ] ViloForge reviewed `git diff` and safety-sensitive files.
- [ ] ViloForge ran canonical tests independently.
