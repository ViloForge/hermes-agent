...
For long-lived processes: ViloForge gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside ViloForge you have two practical options:
...
## Debugging ViloForge-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For ViloForge subagents, debug one process at a time.
