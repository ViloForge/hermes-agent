...
| Version | `2.0.0` |
| Author | ViloForge Agent (writing-craft adapted from obra/superpowers) |
| License | MIT |
...
Treat that as relative to the active working directory / backend workspace. ViloForge file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
...
> **For ViloForge:** Use subagent-driven-development skill to implement this plan task-by-task.
...
Use ViloForge tools to understand the project:
