...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ViloForge Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during ViloForge terminal calls (desktop app flow only)
...
### Service Account (recommended for ViloForge)
...
## ViloForge Execution Pattern (desktop app flow)

ViloForge terminal commands are non-interactive by default and can lose auth context between calls.
