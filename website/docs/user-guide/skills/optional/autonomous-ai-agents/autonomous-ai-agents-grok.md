...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), ViloForge Agent |
| License | MIT |
...
# Grok Build CLI — ViloForge Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the ViloForge terminal. Grok
...
1. Prepare stable input files first with ViloForge tools (`read_file`,
...
2. **Don't conflate ViloForge' xAI auth with the `grok` CLI's auth.** ViloForge'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume ViloForge' xAI auth carries
   over.
