...
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), ViloForge Agent |
| License | MIT |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from ViloForge.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For ViloForge-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (ViloForge subagents via `delegate_task`).
