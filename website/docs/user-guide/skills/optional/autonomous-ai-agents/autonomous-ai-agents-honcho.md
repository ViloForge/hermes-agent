...
description: "Configure and use Honcho memory with ViloForge -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with ViloForge -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
...
# Honcho Memory for ViloForge

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every ViloForge profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this ViloForge profile's AI peer
...
Honcho models conversations as interactions between **peers**. ViloForge creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this ViloForge instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each ViloForge run |
...
Each ViloForge profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for ViloForge when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all ViloForge profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to ViloForge + Honcho |
