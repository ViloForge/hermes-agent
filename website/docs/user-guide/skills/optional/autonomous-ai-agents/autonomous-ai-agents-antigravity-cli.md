...
| Version | `0.1.0` |
| Author | Tony Simons (asimons81), ViloForge Agent |
| License | MIT |
...
commands through the ViloForge `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from ViloForge itself.
