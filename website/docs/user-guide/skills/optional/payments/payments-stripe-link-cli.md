...
| Version | `0.1.0` |
| Author | Teknium (teknium1), ViloForge Agent |
| License | MIT |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so ViloForge can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — ViloForge cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before ViloForge attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "ViloForge" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with ViloForge' native MCP:
