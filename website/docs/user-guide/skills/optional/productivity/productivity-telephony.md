---
title: "Telephony — Give ViloForge phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give ViloForge phone capabilities without core tool changes"
---
...
Give ViloForge phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
This optional skill gives ViloForge practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn ViloForge into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to ViloForge memory
...
### 1) "I want ViloForge to own a real phone number"
Use **Twilio**.
...
- pairs well with ViloForge `text_to_speech` plus a public file host or tunnel
...
Then save credentials into ViloForge:
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is ViloForge calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing ViloForge's existing `text_to_speech` support.

Use this when:
- you want the call to use ViloForge's configured TTS voice rather than Twilio `<Say>`
...
Recommended ViloForge TTS -> Twilio Play workflow:

1. Generate the audio with ViloForge `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask ViloForge to create the message audio with `text_to_speech`
...
Important note:
- ViloForge TTS is great for prerecorded outbound messages
...
- ViloForge STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to ViloForge memory.
...
- Do not store arbitrary third-party phone numbers in ViloForge memory.
