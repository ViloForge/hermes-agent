...
sidebar_position: 3
title: "Desktop App"
description: "The native ViloForge desktop app — a polished experience for chatting with ViloForge, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The ViloForge desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same ViloForge Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
ViloForge has several front ends that all talk to the same agent:
...
Follow the [installation instructions for ViloForge Desktop](../getting-started/installation.md).

If you already have ViloForge installed, simply run
...
- **The same conversation history** as every other ViloForge surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to ViloForge and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader ViloForge management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [ViloForge profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the ViloForge agent, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the ViloForge source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the ViloForge Agent runtime into `HERMES_HOME` (`~/.hermes`, or `%LOCALAPPDATA%\hermes` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `HERMES_DESKTOP_HERMES_ROOT`, then a completed managed install, then a probed `hermes` on `PATH` (unless `--ignore-existing` / `HERMES_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `HERMES_DESKTOP_HERMES` command override for packagers such as Nix. The React renderer talks to a `hermes dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `hermes --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a ViloForge backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting ViloForge Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & ViloForge Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
