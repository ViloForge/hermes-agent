...
sidebar_position: 1
title: "CLI Interface"
description: "Master the ViloForge Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

ViloForge Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
:::tip
ViloForge also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hermes --tui` — see the [TUI](tui.md) guide.
:::
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the ViloForge CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The ViloForge CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend ViloForge to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. ViloForge supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for ViloForge replies |
...
ViloForge loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain ViloForge..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. ViloForge recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches ViloForge — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while ViloForge is working, ViloForge prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend ViloForge to the background — just like any terminal process. The shell prints a confirmation:

```
ViloForge Agent has been suspended. Run `fg` to bring ViloForge Agent back.
```
...
CLI sessions are stored in ViloForge's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
ViloForge immediately confirms the task and gives you back the prompt:
