---
sidebar_position: 6
title: "Use MCP with ViloForge"
description: "A practical guide to connecting MCP servers to ViloForge Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with ViloForge

This guide shows how to actually use MCP with ViloForge Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native ViloForge tool
- you want ViloForge to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect ViloForge to internal APIs, databases, or company systems without modifying ViloForge core

Do not use MCP when:
- a built-in ViloForge tool already solves the job well
...
- ViloForge remains the agent
- MCP servers contribute tools
- ViloForge discovers those tools at startup or reload time
...
If you installed ViloForge with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- ViloForge banner/status should show MCP integration when configured
- ask ViloForge what tools it has available
...
## WSL2: bridge ViloForge in WSL to Windows Chrome

This is the practical setup when:

- ViloForge runs inside WSL2
...
In this setup, ViloForge does **not** connect to Chrome directly. Instead:

- ViloForge runs in WSL
- ViloForge starts a local stdio MCP server
...
```text
ViloForge (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- ViloForge stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on ViloForge core browser transport
...
Then start a fresh ViloForge session or run:
...
Once loaded, ViloForge can use the MCP-prefixed browser tools directly. For example:
...
If ViloForge runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start ViloForge from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start ViloForge from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in ViloForge:
...
2. ViloForge-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So ViloForge will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want ViloForge to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want ViloForge to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. ViloForge remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want ViloForge to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize ViloForge to merge, publish, deploy, or spawn runtimes.
...
Now ViloForge can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing ViloForge core.
...
Because ViloForge now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
