...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in ViloForge Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

ViloForge Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and ViloForge automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in ViloForge — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → ViloForge opens your browser to accounts.x.ai
...
On servers, containers, or SSH sessions where no browser is available, ViloForge detects the remote environment and prints the authorization URL instead of opening a browser.
...
If you don't have a regular SSH client (e.g. you're running ViloForge inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — ViloForge skips the loopback listener and lets you paste the failed callback URL straight from your browser:
...
If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — ViloForge accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.

## How the Login Works

1. ViloForge opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to ViloForge and the tokens are saved to `~/.hermes/auth.json`.
4. From then on, ViloForge refreshes the access token in the background — you stay signed in until you `hermes auth logout xai-oauth` or revoke access from your xAI account settings.
...
ViloForge refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), ViloForge surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), ViloForge marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `hermes auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, ViloForge raises a timeout error.

**Fix:** re-run `hermes auth add xai-oauth` (or `hermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

ViloForge detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions ViloForge prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if ViloForge is on a different machine than your browser
