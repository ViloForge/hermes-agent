...
OAuth needs a browser, but the loopback callback runs on the machine where ViloForge is running. Two options:
...
```
Hey, search the web for "ViloForge Agent release notes" and summarize the top 3 hits.
```

You should see ViloForge call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.
...
These rows appear in `hermes tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, ViloForge runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and ViloForge will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [ViloForge profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so ViloForge doesn't replay it endlessly. Just log in again:
...
- Multiple ViloForge profiles where you're using the wrong one (check `hermes profile list`)
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-ViloForge tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple ViloForge configurations
