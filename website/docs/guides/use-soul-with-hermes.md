---
sidebar_position: 7
title: "Use SOUL.md with ViloForge"
description: "How to use SOUL.md to shape ViloForge Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with ViloForge

`SOUL.md` is the **primary identity** for your ViloForge instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want ViloForge to feel like the same assistant every time you talk to it — or if you want to replace the ViloForge persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm ViloForge should be
- what ViloForge should avoid stylistically
- how ViloForge should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who ViloForge is and how ViloForge speaks
...
ViloForge now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run ViloForge with a custom home directory, it becomes:
...
ViloForge automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, ViloForge does not overwrite it
- if the file exists but is empty, ViloForge adds nothing from it to the prompt

## How ViloForge uses it

When ViloForge starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how ViloForge feels.
...
ViloForge already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How ViloForge should sound.

# Avoid
What ViloForge should not do.

# Defaults
How ViloForge should behave when ambiguity appears.
```
...
Then restart ViloForge or start a new session.
...
4. Talk to ViloForge for a while
...
### I edited SOUL.md but ViloForge still sounds the same
...
### ViloForge is ignoring parts of my SOUL.md
