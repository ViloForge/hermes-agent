...
description: "Diagnose and fix common ViloForge cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the ViloForge config directory). Verify:
```bash
...
hermes logs                         # View recent ViloForge logs
