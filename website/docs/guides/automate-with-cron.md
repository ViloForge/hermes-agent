...
description: "Real-world automation patterns using ViloForge cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
- **Recurring watchdog** where the script already produces the exact message (memory alerts, disk alerts, heartbeats): use [script-only cron jobs](/guides/cron-script-only). Same scheduler, no LLM. You can ask ViloForge to set one up for you in chat — the `cronjob` tool knows when to pick `no_agent=True` and writes the script for you.
...
req = urllib.request.Request(URL, headers={"User-Agent": "ViloForge-Monitor/1.0"})
