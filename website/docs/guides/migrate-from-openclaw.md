...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to ViloForge Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into ViloForge. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | ViloForge destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | ViloForge destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing ViloForge skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | ViloForge destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | ViloForge config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | ViloForge config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | ViloForge field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | ViloForge `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | ViloForge path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct ViloForge equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in ViloForge |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | ViloForge has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use ViloForge profiles |
