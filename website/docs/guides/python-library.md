---
sidebar_position: 5
title: "Using ViloForge as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using ViloForge as a Python Library

ViloForge isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install ViloForge directly from the repository:

```bash
pip install git+https://github.com/ViloForge/hermes-agent.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+https://github.com/ViloForge/hermes-agent.git
```

You can also pin it in your `requirements.txt`:

```text
hermes-agent @ git+https://github.com/ViloForge/hermes-agent.git
```

:::tip
The same environment variables used by the CLI are required when using ViloForge as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use ViloForge is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding ViloForge in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, ViloForge includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
