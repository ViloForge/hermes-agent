...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in ViloForge Agent — no API key required"
---

# MiniMax OAuth

ViloForge Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and ViloForge automatically refreshes your session.
...
# → ViloForge opens your browser to the MiniMax authorization page
...
ViloForge will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

ViloForge implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. ViloForge generates a PKCE verifier / challenge pair and a random state value.
...
4. ViloForge polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
...
ViloForge refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, ViloForge marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), ViloForge marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.
...
The device-code flow has a finite expiry window. If you don't approve the login in time, ViloForge raises a timeout error.

**Fix:** re-run `hermes auth add minimax-oauth` (or `hermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

ViloForge detected that the `state` value returned by the authorization server does not match what it sent.
...
ViloForge prints the URL and code. Open the URL on any device and complete the flow there.
