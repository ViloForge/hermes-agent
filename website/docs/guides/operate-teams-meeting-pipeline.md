...
#### Option 1: ViloForge cron (recommended if you already run the ViloForge gateway)

ViloForge ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
```ini
[Unit]
Description=ViloForge Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run ViloForge Teams pipeline subscription maintenance every 12 hours
...
- [ ] **`maintain-subscriptions` is scheduled** (ViloForge cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
