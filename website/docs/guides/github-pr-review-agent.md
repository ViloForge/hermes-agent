...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  ViloForge Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to ViloForge instantly when PRs are opened or updated.
:::
...
- **ViloForge Agent installed** — see the [Installation guide](/getting-started/installation)
...
Make sure ViloForge can access GitHub. Start a chat:
...
Still in the chat, ask ViloForge to review a real PR:
...
A skill gives ViloForge consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
This is what makes the reviewer actually useful. Start a session and teach ViloForge your team's standards:
...
2. Teach ViloForge your conventions via memory (Step 4)
