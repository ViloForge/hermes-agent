...
messaging platform ViloForge is already configured for. Think of it as a
...
Any platform ViloForge ships adapters for works as a target:
...
When stdin is a TTY (no pipe), ViloForge does **not** wait for input — you'll
...
Because `hermes send` reuses your ViloForge config, the same script works on
any host where ViloForge is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses ViloForge creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
