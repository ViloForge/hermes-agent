...
description: "Find, install, use, and create skills — on-demand knowledge that teaches ViloForge new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach ViloForge how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every ViloForge installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask ViloForge to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with ViloForge but aren't active by default. Install them explicitly:
...
For how to ship skills in your own plugin, see [Build a ViloForge Plugin → Bundle skills](/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, ViloForge prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, ViloForge may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, ViloForge will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell ViloForge to update the skill with what you learned. Skills that aren't maintained become liabilities.
