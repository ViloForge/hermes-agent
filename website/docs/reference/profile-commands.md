...
This page covers all commands related to [ViloForge profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
This shows the profile's ViloForge home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your ViloForge installation.
...
ViloForge version, author, env var requirements, the source URL/path, and
...
ViloForge version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host ViloForge can clone from.
...
Global flag to run any ViloForge command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
