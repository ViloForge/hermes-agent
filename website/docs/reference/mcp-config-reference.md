...
description: "Reference for ViloForge Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
ViloForge may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, ViloForge only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, ViloForge does not create an empty MCP runtime toolset for that server.
...
Behavior:
- ViloForge uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
