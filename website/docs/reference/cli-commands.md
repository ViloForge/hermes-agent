...
description: "Authoritative reference for ViloForge terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which ViloForge profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes backup` | Back up ViloForge home directory to a zip file. |
...
| `hermes import` | Restore a ViloForge backup from a zip file. |
...
| `hermes acp` | Run ViloForge as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run ViloForge as an MCP server. |
| `hermes plugins` | Manage ViloForge Agent plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated ViloForge instances. |
...
| `hermes uninstall` | Remove ViloForge from the system. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from ViloForge itself. |
...
hermes chat --safe-mode -q "Is this bug mine or ViloForge'?"
```
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active ViloForge chat session.
...
**`hermes model`** (run from your terminal, outside any ViloForge session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active ViloForge chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your ViloForge session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `--name NAME` | `ViloForge` | Bot display name in Slack. |
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into ViloForge) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the ViloForge venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hermes/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the ViloForge Python venv. |
...
hermes auth spotify                                      # Authenticate ViloForge with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire ViloForge setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | ViloForge version, release date, git commit hash |
...
The report includes system info (OS, Python version, ViloForge version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Create a zip archive of your ViloForge configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when ViloForge is running (WAL-mode safe).
...
Restore a previously created ViloForge backup into your ViloForge home directory. All files in the archive overwrite existing files in your ViloForge home; `--force` only skips the confirmation prompt that fires when the target already has a ViloForge installation.
...
View, tail, and filter ViloForge log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
ViloForge uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how ViloForge layers
...
- `--source well-known` lets you point ViloForge at a site exposing `/.well-known/skills/index.json`.
...
Starts ViloForge as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run ViloForge as an MCP server.
...
| `serve [-v\|--verbose]` | Run ViloForge as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with ViloForge](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a ViloForge Plugin](../guides/build-a-hermes-plugin.md).
...
right now without waiting for the next ViloForge update.
...
Migrate your OpenClaw setup to ViloForge. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing ViloForge files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into ViloForge equivalents or **archived** for manual review.
...
Manage profiles — multiple isolated ViloForge instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of ViloForge commands, subcommands, and profile names.
...
`hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, ViloForge may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Gateway restart.** After a successful update, ViloForge attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, ViloForge makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
...
- **Legacy `hermes.service` warning.** If ViloForge detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes postinstall` | Internal bootstrap. Runs once after the install script provisions ViloForge (or after `hermes update`) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `hermes setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `hermes uninstall [--full] [--gui] [--yes]` | Remove ViloForge, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |
