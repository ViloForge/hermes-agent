...
description: "Frequently asked questions and solutions to common issues with ViloForge Agent"
---
...
### What LLM providers work with ViloForge?

ViloForge Agent works with any OpenAI-compatible API. Supported providers include:
...
**Yes, natively.** ViloForge supports native Windows via the PowerShell installer — no WSL required. Run in PowerShell:
...
WSL2 remains a fully supported alternative. To run ViloForge inside WSL2, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and use the standard install command:

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
```

### I run ViloForge in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run ViloForge inside WSL2
...
- let ViloForge use the resulting MCP browser tools

This is more reliable than trying to force ViloForge core browser transport to attach directly across the WSL2/Windows boundary.
...
Yes — ViloForge now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). ViloForge Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.
...
# Context length: 64000   ← ViloForge minimum; set this to match your server's actual context window
```
...
ViloForge persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in ViloForge — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
ViloForge auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

ViloForge Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same ViloForge Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use ViloForge programmatically:
...
**Cause:** ViloForge requires Python 3.11 or newer.
...
**Cause:** ViloForge builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when ViloForge runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** ViloForge auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
```bash
# Exit the ViloForge chat session first (Ctrl+C or /quit)
...
**Cause:** The conversation has grown too long for the model's context window, or ViloForge detected the wrong context length for your model.
...
If this happens on the first long conversation, ViloForge may have the wrong context length for your model. Check what it detected:
...
**Cause:** ViloForge detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — ViloForge never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
# Restart ViloForge or reload MCP after config changes
hermes chat
```
...
:::warning
If an MCP server crashes mid-request, ViloForge will report a timeout. Check the server's own logs (not just ViloForge logs) to diagnose the root cause.
:::
...
**Solution: Delegation config.** ViloForge can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell ViloForge "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can ViloForge do this?

**Current limitation:** ViloForge profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, ViloForge reasoning, and tool call details in Telegram instead of just the final output.
...
**Current behavior:** ViloForge creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting ViloForge to another machine
...
1. Install ViloForge Agent on the new machine:
   ```bash
   curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
   ```
...
:::tip
`hermes backup` produces a consistent snapshot even while ViloForge is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the ViloForge installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a ViloForge-specific issue — it's a shell config permissions problem.
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), ViloForge version (`hermes --version`), and the full error message
