...
description: "使用 SuperGrok 或 X Premium+ 订阅登录，在 ViloForge Agent 中使用 Grok 模型——无需 API 密钥"
---

# xAI Grok OAuth（SuperGrok / X Premium+）

ViloForge Agent 通过基于浏览器的 OAuth 登录流程支持 xAI Grok，认证服务器为 [accounts.x.ai](https://accounts.x.ai)，支持 **SuperGrok 订阅**（[grok.com](https://x.ai/grok)）或 **X Premium+ 订阅**（已关联的 X 账号）。无需 `XAI_API_KEY`——登录一次后，ViloForge 会在后台自动刷新会话。
...
同一 OAuth bearer token 也会被 ViloForge 中所有直连 xAI 的功能复用——TTS、图像生成、视频生成和转录——因此单次登录即可覆盖全部四项功能。
...
# → ViloForge 在浏览器中打开 accounts.x.ai
# → 在浏览器中批准访问
...
在没有浏览器的服务器、容器或 SSH 会话中，ViloForge 会检测到远程环境并打印授权 URL，而不是打开浏览器。
...
如果你没有常规 SSH 客户端（例如在 GCP Cloud Shell、GitHub Codespaces、AWS EC2 Instance Connect、Gitpod 或其他基于浏览器的控制台中运行 ViloForge），上述 `ssh -L` 方案不可用。请改用 `--manual-paste`——ViloForge 跳过回环监听器，让你直接从浏览器粘贴失败的回调 URL：
...
1. ViloForge 在浏览器中打开 `accounts.x.ai`。
2. 你登录（或确认现有会话）并批准访问。
3. xAI 重定向回 ViloForge，token 保存到 `~/.hermes/auth.json`。
4. 此后，ViloForge 在后台刷新 access token——你将保持登录状态，直到执行 `hermes auth logout xai-oauth` 或在 xAI 账号设置中撤销访问。
...
ViloForge 在每次会话前刷新 token，并在收到 401 时响应式地再次刷新。如果刷新因 `invalid_grant` 失败（刷新 token 被撤销或账号已轮换），ViloForge 会显示类型化的重新认证消息，而不是崩溃。

当刷新失败是终态时（HTTP 4xx、`invalid_grant`、授权被撤销等），ViloForge 将刷新 token 标记为失效并在本地隔离——后续调用跳过注定失败的刷新尝试，而不是反复重放同一个 401。agent 显示一条"需要重新认证"消息，并在你再次登录前保持等待。

**修复方法：** 再次运行 `hermes auth add xai-oauth` 开始全新登录。下次成功交换后隔离状态自动清除。

### 授权超时

回环监听器有有限的过期窗口（默认 180 秒）。如果你未在时限内批准登录，ViloForge 会抛出超时错误。

**修复方法：** 重新运行 `hermes auth add xai-oauth`（或 `hermes model`）。流程重新开始。

### State 不匹配（可能的 CSRF）

ViloForge 检测到授权服务器返回的 `state` 值与发送的不匹配。

**修复方法：** 重新运行登录。如果问题持续，检查是否有代理或重定向在修改 OAuth 响应。

### 从远程服务器登录

在 SSH 或容器会话中，ViloForge 打印授权 URL 而不是打开浏览器。回环回调监听器仍绑定在远程主机的 `127.0.0.1:56121`——你笔记本上的浏览器无法访问它，除非进行 SSH 本地端口转发：
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — 如果 ViloForge 与浏览器不在同一台机器上，必读
