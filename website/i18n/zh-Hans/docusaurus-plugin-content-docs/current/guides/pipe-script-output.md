...
`hermes send` 是一个轻量、可脚本化的 CLI，能将消息推送到 ViloForge 已配置的任意消息平台。可以把它理解为跨平台的通知专用 `curl`——无需运行中的 gateway，无需 LLM，也无需在每个脚本里重复粘贴 bot token。
...
当 stdin 是 TTY（无管道）时，ViloForge **不会**等待输入——你会收到明确的用法错误提示。这可以防止脚本在意外省略消息体时挂起。
...
由于 `hermes send` 复用你的 ViloForge 配置，同一脚本可在任何安装了 ViloForge 的主机上运行——无需手动将 bot token 导出到每台机器的环境变量中。
...
| 方案 | 多平台 | 复用 ViloForge 凭据 | 需要 gateway | 最适合 |
|----------|----------------|---------------------|---------------|----------|
