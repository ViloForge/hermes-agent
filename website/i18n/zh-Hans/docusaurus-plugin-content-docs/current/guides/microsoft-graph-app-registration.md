...
4. 填写以下内容：
   - **Name：**`ViloForge Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the ViloForge app
New-CsApplicationAccessPolicy `
  -Identity "ViloForge-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict ViloForge meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "ViloForge-Meeting-Pipeline-Policy" `
...
ViloForge 内置了 Graph 身份验证冒烟测试。在 ViloForge 安装目录下执行：
