...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 ViloForge Agent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 OpenClaw 迁移

`hermes claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 ViloForge。本指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
| `--overwrite` | 冲突时覆盖已有 ViloForge 文件（默认：计划存在冲突时拒绝执行）。 |
...
| 内容 | OpenClaw 来源 | ViloForge 目标 | 备注 |
|------|----------------|-------------------|-------|
...
Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 ViloForge skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | ViloForge 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| 内容 | OpenClaw 配置路径 | ViloForge 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
| OpenClaw 配置路径 | ViloForge 配置路径 | 备注 |
|---------------------|-------------------|-------|
...
| 平台 | OpenClaw 配置路径 | ViloForge `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
| 内容 | OpenClaw 路径 | ViloForge 路径 | 备注 |
|------|-------------|-------------|-------|
...
| 多 Agent 列表 | `archive/agents-list.json` | 使用 ViloForge profiles |
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，会从 `~/.openclaw/.env` 和 `openclaw.json` 的 env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会对此发出警告，相关值需通过 `hermes config set` 手动添加至 ViloForge。
