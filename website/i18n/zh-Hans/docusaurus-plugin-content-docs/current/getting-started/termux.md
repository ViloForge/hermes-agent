...
description: "通过 Termux 在 Android 手机上直接运行 ViloForge Agent"
---

# 在 Android 上通过 Termux 运行 ViloForge

这是在 Android 手机上通过 [Termux](https://termux.dev/) 直接运行 ViloForge Agent 的已验证路径。
...
这并不妨碍 ViloForge 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。

---

## 方式一：一行安装命令

ViloForge 现已内置 Termux 感知的安装路径：
