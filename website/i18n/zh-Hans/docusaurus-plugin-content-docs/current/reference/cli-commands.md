...
sidebar_position: 1
title: "CLI 命令参考"
description: "ViloForge 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 ViloForge profile（配置文件）。覆盖 `hermes profile use` 设置的粘性默认值。 |
...
| `hermes acp` | 将 ViloForge 作为 ACP 服务器运行，用于编辑器集成。 |
| `hermes mcp` | 管理 MCP 服务器配置，并将 ViloForge 作为 MCP 服务器运行。 |
| `hermes plugins` | 管理 ViloForge Agent plugin（安装、启用、禁用、删除）。 |
...
交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 ViloForge 聊天会话内部运行。
...
**`hermes model`**（从终端运行，在任何 ViloForge 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 ViloForge 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 ViloForge 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `hermes model`。
:::
...
| `--name NAME` | `ViloForge` | Slack 中的机器人显示名称。 |
...
hermes auth spotify                                      # 通过 PKCE 将 ViloForge 与 Spotify 认证
```
...
输出整个 ViloForge 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | ViloForge 版本、发布日期、git commit hash |
...
报告包含系统信息（操作系统、Python 版本、ViloForge 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
创建 ViloForge 配置、skill、会话和数据的 zip 归档。备份不包含 hermes-agent 代码库本身。
...
备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 ViloForge 正在运行也能正确工作（WAL 模式安全）。
...
将之前创建的 ViloForge 备份恢复到 ViloForge 主目录。归档中的所有文件会覆盖 ViloForge 主目录中的现有文件；`--force` 仅跳过当目标已有 ViloForge 安装时触发的确认提示。
...
查看、跟踪和过滤 ViloForge 日志文件。所有日志存储在 `~/.hermes/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
ViloForge 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`hermes logs list` 子命令显示所有日志文件，包括已轮转的。
...
- `--source well-known` 允许你将 ViloForge 指向暴露 `/.well-known/skills/index.json` 的站点。
...
将 ViloForge 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 ViloForge 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 ViloForge 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 ViloForge 中使用 MCP](../guides/use-mcp-with-hermes.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server)。
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 ViloForge Plugin](../guides/build-a-hermes-plugin.md)。
...
`hermes update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 ViloForge 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 ViloForge。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 ViloForge 文件（默认：当计划有冲突时拒绝应用）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 ViloForge 等效项，要么**归档**以供手动审查。
...
管理 profile——多个隔离的 ViloForge 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 ViloForge 命令、子命令和 profile 名称进行 Tab 补全。
...
- **旧版 `hermes.service` 警告。** 如果 ViloForge 检测到预重命名的 `hermes.service` systemd 单元（而非当前的 `hermes-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| `hermes uninstall [--full] [--yes]` | 删除 ViloForge，可选择删除所有 config/数据。 |
