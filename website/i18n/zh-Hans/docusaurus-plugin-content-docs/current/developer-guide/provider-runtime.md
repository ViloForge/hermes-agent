...
description: "ViloForge 如何在运行时解析 provider、凭据、API 模式及辅助模型"
---

# Provider 运行时解析

ViloForge 拥有一个共享的 provider 运行时解析器，用于以下场景：
...
该顺序很重要，因为 ViloForge 将已保存的模型/provider 选择视为正常运行的真实来源。这可以防止过时的 shell 导出变量悄悄覆盖用户在 `hermes model` 中最后选择的端点。
...
该解析器是 ViloForge 能够在以下场景之间共享认证/运行时逻辑的主要原因：
...
ViloForge 包含相关逻辑，以避免在存在多个 provider 密钥时（例如同时存在 `OPENROUTER_API_KEY` 和 `OPENAI_API_KEY`）将错误的 API key 泄露给自定义端点。
...
当 provider 解析选择 `anthropic` 时，ViloForge 使用：
...
- ViloForge 在调用原生 Messages API 前会预检 Anthropic 凭据刷新
- ViloForge 在重建 Anthropic 客户端后，仍会在收到 401 时重试一次，作为回退路径
...
当辅助任务配置的 provider 为 `main` 时，ViloForge 通过与普通对话相同的共享运行时路径进行解析。实际效果为：
...
ViloForge 支持配置回退 provider 链——一个按顺序尝试的 `(provider, model)` 条目列表，当主模型遇到错误时依次尝试。旧版单对 `fallback_model` 字典仍被接受以保持向后兼容（并在首次写入时迁移）。
