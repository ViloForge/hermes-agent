...
本指南介绍如何向 ViloForge gateway 添加新的消息平台。平台适配器将 ViloForge 连接到外部消息服务（Telegram、Discord、WeCom 等），使用户可以通过该服务与 agent 交互。
...
Plugin 系统允许你在不修改任何 ViloForge 核心代码的情况下添加平台适配器。你的 plugin 是一个包含两个文件的目录：
...
def register(ctx):
    """Plugin 入口点 — 由 ViloForge plugin 系统调用。"""
...
:::note
此清单用于将平台直接添加到 ViloForge 核心代码库 — 通常由核心贡献者为官方支持的平台执行。社区/第三方平台应使用上方的 [Plugin 路径](#plugin-path-recommended)。
:::
