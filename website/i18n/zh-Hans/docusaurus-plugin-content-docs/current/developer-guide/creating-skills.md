...
sidebar_position: 3
title: "创建 Skill"
description: "如何为 ViloForge Agent 创建 skill——SKILL.md 格式、规范与发布"
---

# 创建 Skill

Skill 是为 ViloForge Agent 添加新能力的首选方式。与 tool 相比，skill 更易于创建，无需修改 agent 代码，且可与社区共享。
...
当 skill 需要 API key 或 token 时，使用 `required_environment_variables`。缺少值**不会**将 skill 从发现列表中隐藏。ViloForge 会在本地 CLI 加载 skill 时安全地提示用户输入。
...
用户可以跳过配置并继续加载 skill。ViloForge 不会将原始密钥值暴露给模型。Gateway 和消息会话会显示本地配置指引，而不是在带内收集密钥。
...
加载时，ViloForge 会检查这些文件是否存在。缺少文件会触发 `setup_needed`。已存在的文件会自动：
...
优先使用标准库 Python、curl 以及现有 ViloForge tool（`web_extract`、`terminal`、`read_file`）。若确实需要依赖项，请在 skill 中记录安装步骤。
...
内置 skill（位于 `skills/`）随每次 ViloForge 安装一起发布，应对**大多数用户广泛有用**：
...
信任级别：
- `builtin`——随 ViloForge 一起发布（始终受信任）
...
ViloForge 现在可以通过多种外部发现模型使用第三方 skill：
