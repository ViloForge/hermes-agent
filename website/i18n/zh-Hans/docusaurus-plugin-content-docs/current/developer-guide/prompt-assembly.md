...
sidebar_position: 5
title: "Prompt 组装"
description: "ViloForge 如何构建系统 prompt、保持缓存稳定性并注入临时层"
---
...
| 优先级 | 文件 | 搜索范围 | 说明 |
|--------|------|----------|------|
| 1 | `.hermes.md`、`HERMES.md` | 从 CWD 向上至 git 根目录 | ViloForge 原生项目配置 |
...
大多数用户应将 `agent/prompt_builder.py` 视为实现代码，而非配置入口。推荐的自定义路径是修改 ViloForge 已加载的 prompt 输入，而非直接编辑 Python 模板。
...
- 可选系统 prompt 配置 / API 覆盖 — 添加部署特定的指令文本，无需 fork ViloForge。
...
- 若想改变 ViloForge 为所有人组装 prompt 的方式，修改 Python 代码并将其视为代码贡献
