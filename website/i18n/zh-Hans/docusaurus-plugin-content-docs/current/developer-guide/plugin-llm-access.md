...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——ViloForge 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——ViloForge provider id（如 "openrouter"）
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 ViloForge provider
        # （必须是 ViloForge 已知的 provider——与
...
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 ViloForge 常规的聚合器感知回退流程。
...
现有 `ctx.*` 方法各自扩展一个已有的 ViloForge 子系统：
