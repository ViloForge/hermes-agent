...
sidebar_position: 2
title: "TUI"
description: "启动 ViloForge 的现代终端 UI——支持鼠标操作、丰富的浮层面板和非阻塞输入。"
---

# TUI

TUI 是 ViloForge 的现代前端——一个终端 UI（用户界面），与 [Classic CLI](cli.md) 共享同一 Python 运行时。相同的 agent、相同的会话、相同的斜杠命令；交互界面更简洁、响应更流畅。

这是以交互方式运行 ViloForge 的推荐方式。
...
- **即时首帧** — banner 在应用加载完成前就已渲染，因此 ViloForge 启动时终端不会出现卡顿感。
...
首次启动时，ViloForge 会将 TUI 的 Node 依赖安装到 `ui-tui/node_modules`（一次性操作，耗时数秒）。后续启动速度很快。拉取新版 ViloForge 后，若源文件比 dist 更新，TUI bundle 将自动重新构建。

### 外部预构建

发行版若附带预构建 bundle（如 Nix、系统包），可将 ViloForge 指向该 bundle：
...
状态栏忙碌指示器可插拔——默认在 agent 工作期间每 2.5 秒轮换一次 ViloForge 的 kawaii 表情调色板。通过配置或 `/indicator` 斜杠命令选择不同样式：
...
TUI 遵循所有标准 ViloForge 配置：`~/.hermes/config.yaml`、profile、personality、skin、快捷命令、凭证池、内存提供商、工具/skill 启用状态。不存在 TUI 专属配置文件。
...
如果 TUI 启动失败（无 Node、缺少 bundle、TTY 问题），ViloForge 会打印诊断信息并回退——而不是让你陷入困境。
