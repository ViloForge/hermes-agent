...
sidebar_position: 5
title: "WhatsApp"
description: "通过内置 Baileys 桥接将 ViloForge Agent 设置为 WhatsApp 机器人"
---

# WhatsApp 配置

ViloForge 通过基于 **Baileys** 的内置桥接连接到 WhatsApp。其工作原理是模拟 WhatsApp Web 会话——**而非**通过官方 WhatsApp Business API。无需 Meta 开发者账号或 Business 认证。
...
发生这种情况时，ViloForge 会更新桥接依赖。如果机器人在 WhatsApp 更新后停止工作，
请拉取最新版 ViloForge 并重新配对。
:::
...
- Agent 响应默认以"⚕ **ViloForge Agent**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
| **桥接崩溃或重连循环** | 重启 gateway，更新 ViloForge，如会话因 WhatsApp 协议变更而失效则重新配对。 |
| **WhatsApp 更新后机器人停止工作** | 更新 ViloForge 以获取最新桥接版本，然后重新配对。 |
