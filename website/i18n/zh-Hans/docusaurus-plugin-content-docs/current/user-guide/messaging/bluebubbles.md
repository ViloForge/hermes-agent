...
通过 [BlueBubbles](https://bluebubbles.app/) 将 ViloForge 连接至 Apple iMessage——这是一款免费、开源的 macOS 服务端，可将 iMessage 桥接至任意设备。
...
- ViloForge 与 BlueBubbles 服务端之间的网络连通性
...
**DM 配对（推荐）：**
当有人向你的 iMessage 发送消息时，ViloForge 会自动向其发送配对码。使用以下命令批准：
```bash
...
ViloForge 将连接至你的 BlueBubbles 服务端，注册 webhook，并开始监听 iMessage 消息。
...
- **出站：** ViloForge 通过 BlueBubbles REST API 发送消息。
...
### 聊天寻址
你可以通过邮箱或手机号寻址聊天——ViloForge 会自动将其解析为 BlueBubbles 聊天 GUID，无需使用原始 GUID 格式。
