...
sidebar_position: 12
title: "Google Chat"
description: "使用 Cloud Pub/Sub 将 ViloForge Agent 设置为 Google Chat 机器人"
---

# Google Chat 设置

将 ViloForge Agent 作为机器人接入 Google Chat。该集成使用 Cloud Pub/Sub 拉取订阅接收入站事件，使用 Chat REST API 发送出站消息。与 Slack Socket Mode 或 Telegram 长轮询的使用体验相当：ViloForge 进程无需公网 URL、隧道或 TLS 证书。它直接连接、认证并监听订阅——就像 Telegram 机器人通过 token 监听一样。
...
创建完成后，打开该 SA，进入 **Keys → Add Key → Create new key → JSON**，下载文件。将其保存到只有 ViloForge 可读的位置（例如 `~/.hermes/google-chat-sa.json`，`chmod 600`）。
...
- 投递类型：**Pull**
- 消息保留：**7 天**（这样 ViloForge 重启后积压消息不会丢失）
- 其余保持默认。
...
同时在同一订阅上授予 `Pub/Sub Viewer`——ViloForge 在启动时会调用 `subscription.get()` 进行可达性检查。
...
在浏览器中打开 Google Chat。在 **+ New Chat** 菜单中搜索应用名称，向其发起私信。第一次发消息时，Google 会发送一个 `ADDED_TO_SPACE` 事件，ViloForge 用它来缓存机器人自身的 `users/{id}`，以便过滤自发消息。
...
安装 Google Chat 适配器所需的依赖（目前没有发布 ViloForge extra，请直接安装）：
...
在测试私信中发送"hola"。机器人会先发送一条"ViloForge is thinking…"占位消息，然后原地编辑该消息为真实回复——不会留下"消息已删除"的墓碑。
...
Thread（线程）支持：当用户在 thread 中回复时，ViloForge 会检测 `thread.name` 并在同一 thread 中发送回复，每个 thread 对应独立的 ViloForge 会话。
...
3. 下载 JSON 文件，移动到运行 ViloForge 的宿主机上。
4. 在宿主机上，向 ViloForge 注册该客户端：
...
1. 在控制台检查 Pub/Sub 订阅是否有未投递消息。如果有，说明 ViloForge 未通过认证——验证 `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`，并确认 SA 在订阅上具有 `Pub/Sub Subscriber` 角色。
...
一次性宿主机设置未完成。在运行 ViloForge 的宿主机终端中执行：
...
- **附件下载保护**：ViloForge 只会将 SA bearer token 附加到主机名匹配 Google 自有域名短名单的 URL（`googleapis.com`、`drive.google.com`、`lh[3-6].googleusercontent.com` 等）。其他主机在发起 HTTP 请求前即被拒绝，以防范 SSRF 场景——即精心构造的事件将 bearer token 重定向到 GCE 元数据服务。
