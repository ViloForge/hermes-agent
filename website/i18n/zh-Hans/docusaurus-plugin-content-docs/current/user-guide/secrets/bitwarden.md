...
2. ViloForge 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.hermes/.env` 中。
3. 每次 `hermes`（或 gateway，或 cron 任务）启动时，在加载 `~/.hermes/.env` 之后，ViloForge 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
4. 默认情况下，ViloForge **覆盖**环境中已有的值，因此 Bitwarden 是唯一可信来源——在 Web 应用中轮换一次密钥，每个 ViloForge 进程在下次启动时即可获取最新值。如果希望 `.env` 优先，可在配置中将 `override_existing: false`。
...
2. 创建或选择一个**项目**（例如"ViloForge keys"）。
3. 将提供商密钥添加为 secret。secret 的**名称**将成为环境变量名——使用 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等。
4. **Machine accounts → New machine account → My ViloForge machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
Bitwarden 永远不会阻塞 ViloForge 启动。如果出现任何问题，stderr 会显示一行警告，ViloForge 继续使用 `.env` 中已有的凭据：
...
- 即使 `override_existing: true`，ViloForge 也会拒绝让 Bitwarden 覆盖引导令牌本身。如果你将 `BWS_ACCESS_TOKEN` 作为 secret 存储在项目中，应用时会静默跳过。
- `bws` 二进制文件的下载会与同一 GitHub release 中发布的 SHA-256 校验和进行验证。不匹配时将中止安装。
- 固定版本（撰写本文时为 `bws v2.0.0`）通过向本仓库提交 PR 的方式更新——ViloForge 不会将 `bws` 自动升级到"最新版本"，因为上游 release 的结构可能发生变化。
...
适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 ViloForge 安装进行集中轮换和吊销管理的场景。