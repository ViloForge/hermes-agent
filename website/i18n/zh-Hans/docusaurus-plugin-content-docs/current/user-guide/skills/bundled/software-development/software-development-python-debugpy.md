...
适用于长期运行的进程：ViloForge gateway、tui_gateway、daemon，或已出现异常且无法干净重启的进程。
...
最简便的终端侧 DAP 客户端是 VS Code CLI 或一个小脚本。在 ViloForge 内部有两个实用选项：
...
9. **fork / 多进程。** pdb 不会跟随 fork。每个子进程需要自己的 `breakpoint()` 或 `set_trace()`。对于 ViloForge 子 agent，每次只调试一个进程。
