...
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), ViloForge Agent |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 ViloForge 本身。
...
7. **AGPL 协议。** 永远不要在 ViloForge 核心中使用 `from darwinian_evolver import ...`。`~/.hermes/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
