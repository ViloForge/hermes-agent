...
你也可以让 ViloForge 指向**外部 skill 目录**——与本地目录一起扫描的额外文件夹。参见下方的[外部 Skill 目录](#external-skill-directories)。
...
捆绑的 `plan` skill 是一个很好的示例。运行 `/plan [request]` 会加载该 skill 的指令，告知 ViloForge 在需要时检查上下文、编写 markdown 实现计划而非直接执行任务，并将结果保存在相对于当前工作区/后端工作目录的 `.hermes/plans/` 下。
...
当遇到缺失的值时，ViloForge 仅在本地 CLI 中实际加载 skill 时才会安全地请求输入。你可以跳过设置并继续使用该 skill。消息平台不会在聊天中请求密钥——它们会告诉你改用本地的 `hermes setup` 或 `~/.hermes/.env`。
...
如果你在 ViloForge 之外维护 skills——例如，供多个 AI 工具使用的共享 `~/.agents/skills/` 目录——你可以告诉 ViloForge 也扫描这些目录。
...
- **外部目录不是写保护边界**：如果外部 skill 目录对 ViloForge 进程可写，agent 管理的 skill 更新可以修改该目录中的文件。如果共享的外部 skills 必须保持只读，请使用文件系统权限或单独的 profile/toolset 设置。
...
- **不存在的路径会被静默跳过**：如果配置的目录不存在，ViloForge 会忽略它而不报错。适用于可能不在每台机器上都存在的可选共享目录。
...
| 来源 | 示例 | 说明 |
|--------|---------|-------|
| `official` | `official/security/1password` | ViloForge 随附的可选 skills。 |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `hermes skills search <query> --source skills-sh` 搜索。当 skills.sh slug 与仓库文件夹不同时，ViloForge 会解析别名式 skills。 |
...
ViloForge 目前与以下 skills 生态系统和发现来源集成：

#### 1. 官方可选 skills（`official`）

这些 skills 在 ViloForge 仓库中维护，以内置信任级别安装。
...
这是 Vercel 的公共 skills 目录。ViloForge 可以直接搜索它、查看 skill 详情页、解析别名式 slug，并从底层源仓库安装。
...
ViloForge 可以直接从 GitHub 仓库和基于 GitHub 的 tap 安装。当你已知仓库/路径或想添加自己的自定义源仓库时非常有用。
...
- ViloForge 来源 id：`clawhub`

#### 6. Claude 市场式仓库（`claude-marketplace`）

ViloForge 支持发布 Claude 兼容插件/市场清单的市场仓库。

已知集成来源包括：
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

ViloForge 来源 id：`claude-marketplace`

#### 7. LobeHub（`lobehub`）

ViloForge 可以从 LobeHub 的公共目录中搜索并将 agent 条目转换为可安装的 ViloForge skills。
...
- ViloForge 来源 id：`lobehub`

#### 8. browse.sh（`browse-sh`）

ViloForge 与 [browse.sh](https://browse.sh) 集成，这是 Browserbase 的目录，包含 200+ 个针对特定站点的浏览器自动化 SKILL.md 文件（Airbnb、Amazon、arXiv、12306.cn、Etsy、Xero 等）。每个 skill 描述如何端到端驱动一个网站，适合与 ViloForge 的浏览器工具以及你已安装的任何浏览器自动化 skills 配合使用。

- 站点：[browse.sh](https://browse.sh/)
- 目录 API：`https://browse.sh/api/skills`
- ViloForge 来源 id：`browse-sh`
- 信任级别：`community`
...
直接从任何 HTTP(S) URL 安装单文件 `SKILL.md`——当作者在自己的站点上托管 skill 时非常有用（无 hub 列表，无需输入 GitHub 路径）。ViloForge 获取 URL，解析 YAML frontmatter，进行安全扫描并安装。
...
如果你想分享一组精选的 skills——为你的团队、组织或公开分享——你可以将它们发布为 **tap**：其他 ViloForge 用户通过 `hermes skills tap add <owner/repo>` 添加的 GitHub 仓库。无需服务器，无需注册表注册，无需发布流水线。只需一个包含 `SKILL.md` 文件的目录。
...
ViloForge 通过列出 tap 路径的每个子目录并探测每个目录中的 `SKILL.md` 来发现 skills。
...
将其推送到 GitHub 后，任何 ViloForge 用户都可以订阅并安装：
...
新 tap 默认分配 `community` 信任级别。从中安装的 skills 经过标准安全扫描，首次安装时显示第三方警告面板。如果你的组织或广泛受信任的来源应获得更高信任，请将其仓库添加到 `tools/skills_hub.py` 中的 `TRUSTED_REPOS`（需要 ViloForge 核心 PR）。
...
ViloForge 在仓库的 `skills/` 中附带一组捆绑 skills。在安装时以及每次 `hermes update` 时，同步过程会将这些 skills 复制到 `~/.hermes/skills/` 中，并在 `~/.hermes/skills/.bundled_manifest` 记录一个清单，将每个 skill 名称映射到同步时的内容哈希（**origin hash**）。

每次同步时，ViloForge 重新计算本地副本的哈希并与 origin hash 比较：
