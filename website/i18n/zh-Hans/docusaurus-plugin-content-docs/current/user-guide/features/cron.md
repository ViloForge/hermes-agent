...
使用自然语言或 cron 表达式调度自动运行的任务。ViloForge 通过单一 `cronjob` 工具暴露 cron 管理能力，采用动作式操作，而非分散的 schedule/list/remove 工具。
...
所有这些功能均可通过 `cronjob` 工具由 ViloForge 自身使用，因此你可以用自然语言创建、暂停、编辑和删除任务——无需 CLI。

:::warning
Cron 运行的会话不能递归创建更多 cron 任务。ViloForge 在 cron 执行内部禁用了 cron 管理工具，以防止失控的调度循环。
:::
...
ViloForge 会在内部使用统一的 `cronjob` 工具。
...
默认情况下，cron 任务继承创建它的 gateway/CLI 所属的 ViloForge profile。传入 `--profile <name>`（CLI）或 `profile=`（cronjob 工具）可将任务重定向到不同的 profile——调度器会解析该 profile 的 `HERMES_HOME`，在运行期间临时切换到该 profile，加载其 `.env` 和 `config.yaml`，并在其中执行任务：
...
使用 `--profile default` 可显式固定到根 ViloForge profile。指定的 profile 必须已存在；调度器不会动态创建 profile。在 `cron edit` 时清除 profile 固定，传入空字符串（`--profile ""` 或 `profile=""`）——任务将恢复在调度器当前所在的 profile 中运行。
...
`cronjob` 工具的 schema 直接向 ViloForge 暴露了 `no_agent`，因此你可以在聊天中描述一个看门狗，让 agent 来配置它：

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

ViloForge 会通过 `write_file` 将检查脚本写入 `~/.hermes/scripts/`，然后调用：
...
- 任务 2 触发时，ViloForge 从 `~/.hermes/cron/output/{job1_id}/*.md` 读取任务 1 的最新输出
...
Agent 的最终响应会自动投递——你**无需**在 cron prompt 中为同一目标包含 `send_message`。如果 cron 运行调用了 `send_message` 且目标与调度器已投递的目标完全相同，ViloForge 会跳过该重复发送，并告知模型将面向用户的内容放在最终响应中。仅对额外或不同的目标使用 `send_message`。
...
当任务上设置了 `enabled_toolsets` 时，它优先生效；否则 `hermes tools` 的 cron 平台配置生效；否则 ViloForge 回退到内置默认值。这对成本控制很重要：在每个小型"获取新闻"任务中携带 `moa`、`browser`、`delegation` 会在每次 LLM 调用时膨胀工具 schema prompt。

### 完全跳过 agent：`wakeAgent`

如果你的 cron 任务附加了预检脚本（通过 `script=`），脚本可以在运行时决定 ViloForge 是否应该调用 agent。在 stdout 最后一行输出如下格式：
...
:::tip
ViloForge 自身的 `~/.hermes/state.db` 是内部 schema，会在版本间变更。不要从预运行门控中查询它——指向你自己的数据库或 feed。
:::
...
任务可能将 `model` 和 `provider` 存储为 `null`。省略这些字段时，ViloForge 在执行时从全局配置中解析它们。只有设置了单任务覆盖时，这些字段才会出现在任务记录中。
