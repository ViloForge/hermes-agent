...
凭证池允许你为同一提供商注册多个 API 密钥或 OAuth 令牌。当某个密钥触达速率限制或计费配额时，ViloForge 会自动轮换到下一个健康密钥——在不切换提供商的情况下保持会话持续运行。
...
如果你已在 `.env` 中设置了 API 密钥，ViloForge 会自动将其识别为单密钥池。要充分利用池化功能，请添加更多密钥：
...
ViloForge 在启动时自动从多个来源发现凭证并初始化池：
...
| ViloForge PKCE OAuth | `~/.hermes/auth.json` | 是（Anthropic） |
