...
ViloForge 以后台子进程方式运行完整的语言服务器——pyright、gopls、rust-analyzer、
...
这与顶级编码 agent 所采用的架构相同。ViloForge 将其作为自包含组件提供：
...
ViloForge 会自动检测 PATH 上或 `<HERMES_HOME>/lsp/bin/` 中的二进制文件。
...
当 `install_strategy: auto` 时，ViloForge 将二进制文件安装到 `<HERMES_HOME>/lsp/bin/`。
...
服务器在 ViloForge 进程的整个生命周期内保持运行。没有空闲超时回收机制——每次写入都
