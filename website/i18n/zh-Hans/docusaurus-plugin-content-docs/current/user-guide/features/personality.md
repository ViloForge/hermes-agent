...
sidebar_position: 9
title: "个性与 SOUL.md"
description: "通过全局 SOUL.md、内置个性预设和自定义角色定义来自定义 ViloForge Agent 的个性"
---

# 个性与 SOUL.md

ViloForge Agent 的个性完全可自定义。`SOUL.md` 是**主要身份标识**——它是系统提示词（prompt）中的第一项内容，定义了 Agent 是谁。

- `SOUL.md` — 存放在 `HERMES_HOME` 中的持久角色文件，作为 Agent 的身份标识（系统提示词中的第 1 个槽位）
- 内置或自定义的 `/personality` 预设 — 会话级系统提示词覆盖层

如果你想改变 ViloForge 的身份，或将其替换为完全不同的 Agent 角色，请编辑 `SOUL.md`。

## SOUL.md 的工作方式

ViloForge 现在会自动在以下位置生成默认的 `SOUL.md`：

```text
~/.hermes/SOUL.md
```

更准确地说，它使用当前实例的 `HERMES_HOME`，因此如果你以自定义主目录运行 ViloForge，它将使用：
...
- 如果 `SOUL.md` 尚不存在，ViloForge 会自动创建一个初始文件
- 已有的用户 `SOUL.md` 文件不会被覆盖
- ViloForge 仅从 `HERMES_HOME` 加载 `SOUL.md`
- ViloForge 不会在当前工作目录中查找 `SOUL.md`
- 如果 `SOUL.md` 存在但为空，或无法加载，ViloForge 将回退到内置的默认身份
...
如果 ViloForge 从你启动它的任意目录加载 `SOUL.md`，你的个性可能会在不同项目之间意外改变。通过仅从 `HERMES_HOME` 加载，个性归属于 ViloForge 实例本身。

这也让用户更容易理解：
- "编辑 `~/.hermes/SOUL.md` 来更改 ViloForge 的默认个性。"
...
如果文件为空、仅含空白字符或无法读取，ViloForge 将回退到内置默认身份（"You are ViloForge Agent, an intelligent AI assistant created by Nous Research..."）。当 `skip_context_files` 被设置时（例如在子 Agent/委托上下文中），同样适用此回退。
...
ViloForge 内置了多种个性，可通过 `/personality` 切换。
...
这些是便捷的覆盖层，但你的全局 `SOUL.md` 仍然赋予 ViloForge 持久的默认个性，除非覆盖层对其进行了实质性更改。
...
- `SOUL.md`、`agent.system_prompt` 和 `/personality` 影响 ViloForge 的说话方式
- `display.skin` 和 `/skin` 影响 ViloForge 在终端中的显示外观
