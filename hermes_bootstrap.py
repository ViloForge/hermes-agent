"""Windows UTF-8 bootstrap for ViloForge entry points.
...
should be imported at the very top of every ViloForge entry point
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing ViloForge modules.

    ViloForge ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real ViloForge source root
