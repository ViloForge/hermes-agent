# ViloForge Observer Hooks

ViloForge observer hooks are the read-only telemetry contract for plugins that
...
They do not replace ViloForge' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. ViloForge catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so ViloForge only builds
