...
ViloForge uses OpenAI/httpx and requests-based clients for provider calls, model metadata, gateway delivery, and web tools. Those clients inherit CA bundle settings from:
...
When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before ViloForge has enough context to produce a useful message.
...
For a normal corrupted ViloForge venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if ViloForge should use the bundled `certifi` store.
