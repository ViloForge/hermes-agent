...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_hermes_variants(self, monkeypatch):
        ...
        assert adapter._message_matches_mention_patterns("Hermes, summarize this")  # completeness-gate: keep (functional wake word)
        assert adapter._message_matches_mention_patterns("@Hermes agent help")  # completeness-gate: keep (functional wake word)
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Hermes what is next?")  # completeness-gate: keep (functional wake word)

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Hermes, summarize this") == "summarize this"  # completeness-gate: keep (functional wake word)
        assert adapter._clean_mention_text("Hermes agent: summarize this") == "summarize this"  # completeness-gate: keep (functional wake word)
        assert adapter._clean_mention_text("please ask Hermes about this") == "please ask Hermes about this"  # completeness-gate: keep (functional wake word)
...
class TestBlueBubblesMentionGating:
    ...
    async def test_group_message_with_default_mention_is_dispatched_cleaned(self, monkeypatch):
        ...
        response = await adapter._handle_webhook(_FakeBlueBubblesRequest({
            ...
            "data": {
                "guid": "msg-2",
                "text": "Hermes, summarize this",  # completeness-gate: keep (functional wake word)
                "handle": {"address": "+15555550100"},
                "isFromMe": False,
                "isGroup": True,
                "chats": [{"guid": "iMessage;+;group-chat"}],
