...
class TestIsBotMentioned:
    ...
    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@hermes:example.org">ViloForge</a> help'
        assert self.adapter._is_bot_mentioned("ViloForge help", html)
...
class TestStripMention:
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "ViloForge Agent" should not be mutated.
        result = self.adapter._strip_mention("ViloForge Agent")
        assert result == "ViloForge Agent"
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@hermes:example.org">ViloForge</a> help'
    event = _make_event("ViloForge help", formatted_body=formatted)
