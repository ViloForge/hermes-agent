...
Verifies that ViloForge-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed ViloForge secrets.
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a ViloForge inference secret
        ...
        models`` run inside a ViloForge terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        ViloForge-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their ViloForge-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to ViloForge, and blocklisting
