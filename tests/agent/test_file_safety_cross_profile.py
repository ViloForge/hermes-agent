"""Tests for the cross-ViloForge-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another ViloForge profile's
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake ViloForge root with two profiles, monkeypatch the
...
class TestClassifyCrossProfileTarget:
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any ViloForge root
