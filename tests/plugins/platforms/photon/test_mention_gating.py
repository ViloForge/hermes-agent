...
def test_require_mention_defaults_off(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Defaults compile to the two Hermes wake-word patterns.  # completeness-gate: keep (functional wake word)
...
async def test_group_message_passes_and_strips_wake_word(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await adapter._dispatch_inbound(_group_payload("Hermes what's the weather"))  # completeness-gate: keep (functional wake word)
