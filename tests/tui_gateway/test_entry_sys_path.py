...
ViloForge's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the ViloForge root.  harden_import_path must relocate the
    ViloForge root to the front so ``from utils import ...`` resolves to ViloForge."""
    import sys
