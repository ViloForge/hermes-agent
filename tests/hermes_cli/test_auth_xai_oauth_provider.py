"""Tests for xAI Grok OAuth — tokens stored in ViloForge auth store (~/.hermes/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the ViloForge auth store at the given root."""
...
def test_xai_oauth_authorize_url_includes_referrer_hermes_agent():
    """Attribution: xAI's OAuth server can identify ViloForge-originated logins
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    ViloForge that didn't validate) must not be silently honored on the refresh
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every ViloForge invocation."
    )
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another ViloForge process refreshed the singleton
