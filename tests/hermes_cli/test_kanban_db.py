...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real ViloForge profile — preserves the
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "ViloForge-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """ViloForge' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` ViloForge' DB/metadata/logs on task completion.
    """
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "ViloForge Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
