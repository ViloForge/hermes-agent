...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "ViloForge is not logged into Nous Portal").
    ...
    # doesn't raise "ViloForge is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", ViloForge must surface an
    ...
    re-authenticate" string led users to report this as a ViloForge persistence
