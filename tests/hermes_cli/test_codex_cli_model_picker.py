...
Covers:
 - Normal path (tokens already in ViloForge auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — ViloForge
...
def hermes_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in ViloForge auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hermes_auth_only_env):
    """openai-codex appears when tokens are already in ViloForge auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or ViloForge
    auth store."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
