...
only when they still resolve into the ViloForge-managed node dir.
"""
...
def test_removes_dangling_symlink_into_hermes_node(fake_home):
    """A link into the ViloForge node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the ViloForge links that exist; ignores the ones that don't."""
    hermes_home = fake_home / ".hermes"
    node_bin = _make_hermes_node(hermes_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are ViloForge-managed; node is a real user binary.
