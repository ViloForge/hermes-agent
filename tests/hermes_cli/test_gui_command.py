...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "ViloForge.app" / "Contents" / "MacOS" / "ViloForge"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "ViloForge.exe"
    else:
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "ViloForge.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "ViloForge.exe"
