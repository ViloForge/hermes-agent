# ViloForge Agent Security Policy

This document describes ViloForge Agent's trust model, names the one
...
security vulnerabilities. **ViloForge Agent does not operate a bug
bounty program.**
...
ViloForge Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running ViloForge Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted ViloForge Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in ViloForge Agent's documentation
...
ViloForge Agent supports two OS-level isolation postures. They address
...
ViloForge Agent supports this in two ways:

- **ViloForge Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, ViloForge Agent's in-process heuristics
...
ViloForge Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in ViloForge Agent
itself. Bugs in ViloForge Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   ViloForge Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, ViloForge Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  ViloForge Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not ViloForge Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in ViloForge Agent. Bugs in ViloForge
...
- ViloForge Agent includes supply-chain guards for MCP server
