"""On-demand supply-chain audit for ViloForge Agent installs.

Scans three surfaces a ViloForge user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The ViloForge venv (every PyPI dist via ``importlib.metadata``).
