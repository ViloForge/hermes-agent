...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "ViloForge may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a ViloForge gateway is running with connected platforms.
    ...
    print()
    print_error(
        "ViloForge gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to ViloForge")
...
def _cmd_migrate(args):
    """Run the OpenClaw → ViloForge migration."""
    ...
    # ── Phase 2b: Pre-apply backup of the ViloForge home ─────────
    ...
    if not no_backup:
        try:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the ViloForge home."
            )
