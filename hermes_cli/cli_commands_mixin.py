...
class CLICommandsMixin:
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for ViloForge config/state.
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a ViloForge-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"ViloForge keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while ViloForge is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while ViloForge is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while ViloForge is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update ViloForge Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update ViloForge Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update ViloForge Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update ViloForge Agent",
