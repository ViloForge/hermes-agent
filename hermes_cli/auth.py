"""
Multi-provider authentication system for ViloForge Agent.
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in ViloForge.
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but ViloForge times out") can produce
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize ViloForge:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"ViloForge is waiting for the OAuth callback on {redirect_uri}")
...
#
# ViloForge maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from ViloForge auth store (~/.hermes/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to ViloForge auth store (~/.hermes/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into ViloForge auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating ViloForge auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to ViloForge auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. ViloForge keeps its OWN
        ...
        # the Codex CLI (or another ViloForge process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from ViloForge's own Codex token store.
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other ViloForge processes
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    # written by an older ViloForge (or hand-edited) may carry a non-xAI
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's ViloForge root, refuse rather than silently corrupt cross-profile
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with ViloForge's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, ViloForge's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another ViloForge install sharing "
            "~/.hermes/auth.json) called POST /api/oauth/token with ViloForge's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only ViloForge may call the "
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip ViloForge models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "ViloForge is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("ViloForge is not logged into Nous Portal.",
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on ViloForge' "
...
def _login_openai_codex(
    ...
    *,
    ...
    # Check for existing ViloForge-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in ViloForge auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("ViloForge will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("ViloForge will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — ViloForge gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(ViloForge creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to ViloForge auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in ViloForge auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(ViloForge creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=hermes-agent` lets xAI attribute ViloForge-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in ViloForge — please report at "
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize ViloForge with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to ViloForge auth store (~/.hermes/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting ViloForge login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting ViloForge login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older ViloForge builds still see
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("ViloForge will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hermes model` or configure an API key to use ViloForge.")
        else:
