"""
ViloForge Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/ViloForge.app`` or ``~/Applications/ViloForge.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\ViloForge`` (NSIS per-user)
...
the app name ("ViloForge"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/ViloForge``
  - Windows: ``%APPDATA%\\ViloForge``
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "ViloForge"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "ViloForge"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "ViloForge"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "ViloForge" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/ViloForge.app"),
            home / "Applications" / "ViloForge.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\ViloForge).
            local_base / "Programs" / "ViloForge",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "ViloForge")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "ViloForge.desktop",
        ]
    return paths
