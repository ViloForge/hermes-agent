"""Migrate ViloForge' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads ViloForge' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  ViloForge mcp_servers.<n>.command/args/env  → codex stdio transport
  ViloForge mcp_servers.<n>.url/headers       → codex streamable_http transport
  ViloForge mcp_servers.<n>.timeout           → codex tool_timeout_sec
  ViloForge mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  ViloForge-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. ViloForge' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in ViloForge config.")
...
# ViloForge keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # ViloForge' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one ViloForge MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # ViloForge' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown ViloForge key)")
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by ViloForge)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert ViloForge' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before ViloForge' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior ViloForge
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches ViloForge' own
...
def migrate(
    ...
    *,
    ...
    """Translate ViloForge mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.
    ...
    Args:
        ...
        expose_hermes_tools: when True (default), register ViloForge' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into ViloForge for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in ViloForge config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject ViloForge' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into ViloForge for the tools codex
