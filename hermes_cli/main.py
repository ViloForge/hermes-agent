#!/usr/bin/env python3
"""
ViloForge CLI - Main entry point.
...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → ViloForge + Honcho
    hermes version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall ViloForge Agent
...
def _print_fast_version_info() -> None:
    ...
    print(f"ViloForge Agent v{__version__} ({__release_date__})")
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to ViloForge' own profile selector.
        """
        try:
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether ViloForge itself has been explicitly configured (model
    ...
    # Only count these if ViloForge has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants ViloForge to use their tokens.
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this ViloForge checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `hermes update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the ViloForge checkout, run `git restore -- ui-tui`\n"
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with ViloForge — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. ViloForge still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like ViloForge isn't configured yet -- no API keys or providers found."
        )
        print()
    ...
    # setup (config, rules files, plugins, MCP servers) or from ViloForge itself.
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ViloForge?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ViloForge Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_postinstall(args):
    ...
    print("⚕ ViloForge post-install bootstrap")
    print()
...
#
# ViloForge uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  ViloForge only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use ViloForge URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    ViloForge will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate ViloForge CLI with a provider."""
...
def cmd_backup(args):
    """Back up ViloForge home directory to a zip file."""
...
def cmd_import(args):
    """Restore a ViloForge backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall ViloForge Agent (or just the Chat GUI with --gui)."""
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/ViloForge.app/Contents/MacOS/ViloForge"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "ViloForge.exe",
            release_dir / "win-ia32-unpacked" / "ViloForge.exe",
            release_dir / "win-arm64-unpacked" / "ViloForge.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "ViloForge",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "ViloForge",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/ViloForge'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``ViloForge.exe`` keeps an exclusive lock on
    ``release/win-unpacked/ViloForge.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\ViloForge.exe: Access is
    ...
    an unrelated "ViloForge" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "ViloForge is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../ViloForge.app/Contents/MacOS/ViloForge  ->  app bundle = .../ViloForge.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ ViloForge Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ ViloForge Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds ViloForge.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose ViloForge.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on ViloForge.exe, close any")
                    print("  running ViloForge desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "ViloForge is
    ...
    if source_mode:
        print("→ Launching ViloForge Desktop from source build...")
    ...
    print(f"→ Launching packaged ViloForge Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the ViloForge Desktop Electron app to protect its own
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the ViloForge Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    """Update ViloForge Agent by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ViloForge behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but ViloForge couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but ViloForge couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if ViloForge behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but ViloForge couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but ViloForge couldn't drop "
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ViloForge repository.")
...
def _detect_concurrent_hermes_instances(
    ...
    The ViloForge Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-ViloForge parent (e.g. a ViloForge
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close ViloForge Desktop, exit any open `hermes` REPLs, and")
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the ViloForge Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       ViloForge Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close ViloForge Desktop, exit other `hermes` REPLs, stop the "
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# ViloForge Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating ViloForge...")
    try:
...
def cmd_update(args):
    """Update ViloForge Agent to the latest version.
    ...
    if is_managed():
        managed_error("update ViloForge Agent")
        return
...
def _cmd_update_pip(args):
    """Update ViloForge via pip (for PyPI installs)."""
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating ViloForge Agent...")
    print()
    ...
    try:
        ...
        # Warn if legacy ViloForge gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy ViloForge gateway unit(s) detected:")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     ViloForge {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: ViloForge {mf.hermes_requires}")
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other ViloForge surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_logs(args):
    """View and filter ViloForge log files."""
...
def cmd_acp(args):
    """Launch ViloForge Agent as an ACP server."""
    try:
...
def main():
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and ViloForge shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the ViloForge app. `status` reports the driver's grant\n"
    ...
    # for one release. The ViloForge-Setup.exe success screen tells users to
