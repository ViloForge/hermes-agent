...
Diagnoses issues with ViloForge Agent setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 ViloForge Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses ViloForge' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give ViloForge a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# ViloForge Agent Persona\n\n"
                "<!-- Edit this file to customize how ViloForge communicates. -->\n\n"
                "You are ViloForge, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a ViloForge one.
                        check_info(
