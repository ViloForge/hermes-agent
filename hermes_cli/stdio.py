...
on Japanese locales, etc.).  ViloForge's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. ViloForge is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the ViloForge-managed Git install directories so
    ...
    # install without ViloForge-managed PortableGit).
    ...
    # stdin is re-configured for completeness; ViloForge's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and ViloForge's
    ...
    Set this before launching ViloForge (User env var in Windows Settings, or
    export in a PowerShell profile) and ViloForge picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known ViloForge-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess ViloForge spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known ViloForge-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # ViloForge venv Scripts directory — host of the hermes.exe shim itself,
