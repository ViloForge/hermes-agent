...
def remove_path_from_shell_configs():
    """Remove ViloForge PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# ViloForge Agent" comment and following line
                if '# ViloForge Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this ViloForge home's ``node``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify ViloForge-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip ViloForge-owned entries from User-scope PATH in the registry.
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ ViloForge Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No ViloForge Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The ViloForge agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The ViloForge agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The ViloForge agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ViloForge Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ViloForge data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the ViloForge code but keep your configuration and data.")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No ViloForge-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No ViloForge-set User env vars to remove")
    ...
    #     (only when they still point into this ViloForge home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing ViloForge-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No ViloForge-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using ViloForge Agent! ⚕")
    print()
