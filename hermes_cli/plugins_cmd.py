...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other ViloForge subprocess resolution: :func:`shutil.which` first,
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update ViloForge.",
                ) from None
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid ViloForge plugin.",
        )
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as ViloForge
