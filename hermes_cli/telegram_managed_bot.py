...
manual BotFather token copy-paste. ViloForge talks only to the Nous onboarding
...
DEFAULT_BOT_NAME = "ViloForge Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting ViloForge Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the ViloForge Telegram onboarding service.")
