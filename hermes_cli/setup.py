"""
Interactive setup wizard for ViloForge Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ ViloForge Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure ViloForge using environment variables or config commands:")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the ViloForge auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where ViloForge runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where ViloForge delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects ViloForge to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with ViloForge from anywhere.")
...
# Config values may have different semantics between OpenClaw and ViloForge.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure ViloForge to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point ViloForge at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point ViloForge at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point ViloForge at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point ViloForge at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to ViloForge equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing ViloForge config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in ViloForge.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ ViloForge's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("ViloForge can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing ViloForge configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing ViloForge config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in ViloForge (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    ViloForge session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ ViloForge Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ ViloForge Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ ViloForge Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your ViloForge Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have ViloForge configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up ViloForge?",
            [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect ViloForge to messaging apps to chat from anywhere.")
