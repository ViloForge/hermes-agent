...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this ViloForge install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this ViloForge understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their ViloForge is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this ViloForge
        understands. Update ViloForge to install this entry.
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    print(color(
        ...
        f"Start a new ViloForge session to load its tools.",
        Colors.GREEN,
    ))
