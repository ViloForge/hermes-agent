"""
Single source of truth for provider identity in ViloForge Agent.
...
2. **ViloForge overlays** — transport type, auth patterns, aggregator flags,
...
# -- ViloForge overlay ----------------------------------------------------------
# ViloForge-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """ViloForge-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. ViloForge overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + ViloForge overlay
    ...
    if overlay is not None:
        # ViloForge-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
