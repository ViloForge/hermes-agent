"""
Configuration management for ViloForge Agent.
...
#   the next ``subprocess.run([...])`` ViloForge makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. ViloForge itself starts
...
#   ``HERMES_ENV`` — ViloForge runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # ViloForge runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or ViloForge runtime location "
...
def is_managed() -> bool:
    """Check if ViloForge is running in package-manager-managed mode.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how ViloForge was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* ViloForge lives in a ``uv tool`` layout.
    ...
    the *active* ViloForge is a regular pip/venv install, causing
...
#     — that script installs a *new* host-side ViloForge, it doesn't update
...
ViloForge Agent runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this ViloForge installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this ViloForge installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this ViloForge installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this ViloForge installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall ViloForge."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running ViloForge commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When ViloForge runs in a container with volume-mounted config files, forcing
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the ViloForge-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps ViloForge
        ...
        # app, ACP) in a code workspace, ViloForge adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, ViloForge auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), ViloForge sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when ViloForge
        ...
        # bundled ViloForge image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, ViloForge sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and ViloForge should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before ViloForge applies
    ...
    # single read_file page exceeds these limits, ViloForge truncates the
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; ViloForge appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # response. If not, ViloForge feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before ViloForge auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ViloForge Agent*" header.
    ...
    # replace ViloForge' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        "acked_advisories": [],
        # Allow ViloForge to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core ViloForge tools (terminal, read_file, write_file, patch,
    ...
    "gateway": {
        ...
        # behavior: files must live under the ViloForge cache, under
        ...
        # uploaded as native gateway attachments. Files inside the ViloForge
    ...
    "computer_use": {
        ...
        # by default upstream. ViloForge disables it for our users unless they
        # explicitly opt in here. When false (default), ViloForge sets
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the ViloForge auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use ViloForge, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote ViloForge API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote ViloForge API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote ViloForge API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older ViloForge versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — ViloForge won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real ViloForge env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to ViloForge — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known ViloForge vars that are no longer in .env
...
def show_config():
    ...
    print(color("│              ⚕ ViloForge Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
