"""Profile distributions — shareable, packaged ViloForge profiles via git.

A distribution is a ViloForge profile published as a git repository (or
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires ViloForge {op}{target}, "
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a ViloForge profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a ViloForge distribution."
        )
