"""Slash command definitions and autocomplete for the ViloForge CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of ViloForge config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal ViloForge works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while ViloForge is working", "Configuration",
    ...
    CommandDef("update", "Update ViloForge Agent to the latest version", "Info"),
    CommandDef("version", "Show ViloForge Agent version", "Info", aliases=("v",)),
...
# Telegram allows up to 100 BotCommands. ViloForge ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full ViloForge
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to ViloForge or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
