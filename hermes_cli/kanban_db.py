...
``<root>`` is the **shared ViloForge root** (the parent of any active
...
the profile's ViloForge home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared ViloForge root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources ViloForge treats as user-controlled-but-trusted on the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold ViloForge' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside ViloForge-managed storage. A board ``default_workdir`` pointing at a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the ViloForge checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, ViloForge treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, ViloForge creates/reuses that linked worktree. With
      no ``workspace_path``, ViloForge anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, ViloForge uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a ViloForge
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real ViloForge profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real ViloForge profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a ViloForge
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound ViloForge CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved ViloForge shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved ViloForge executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       ViloForge is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install ViloForge Agent or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default ViloForge root exists
