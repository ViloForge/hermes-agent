#!/usr/bin/env node
/**
 * ViloForge Agent WhatsApp Bridge
 *
...
const DEFAULT_REPLY_PREFIX = '⚕ *ViloForge Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['ViloForge Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore ViloForge' own reply messages in self-chat mode to avoid loops.
