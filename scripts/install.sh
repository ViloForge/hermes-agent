...
# Python-driven tool session (e.g. ViloForge terminal tool). A pre-set PYTHONPATH
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "ViloForge Agent Installer"
            echo ""
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> ViloForge.app)"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, ViloForge installs the code under"
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (ViloForge-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download ViloForge Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Point a ViloForge-managed Node's `npm install -g` at a directory that is on
...
# ViloForge's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no ViloForge-managed npm, so calling it on
...
install_uv() {
    ...
    # ViloForge owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
...
# string clears that floor; anything below it is replaced with the ViloForge-
...
check_node() {
    ...
    # Repair pre-existing ViloForge-managed installs where `npm install -g` lands
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ] && node_satisfies_build "$("$HERMES_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$HERMES_HOME/node/bin/node" --version) found (ViloForge-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing ViloForge-managed Node $NODE_VERSION LTS..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. ViloForge install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "ViloForge Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if ViloForge behaves unexpectedly."
                    else
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# ViloForge Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# ViloForge Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# ViloForge Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# ViloForge Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "ViloForge will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# ViloForge Agent browser tools — explicit browser override."
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for ViloForge to send/receive messages."
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up ViloForge for pip install"
...
# extract a tree MISSING the electron binary, so the `electron`->`ViloForge` rename
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the ViloForge-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/ViloForge" ]; then
            app="$desktop_dir/release/linux-unpacked/ViloForge"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/ViloForge.app" \
            "$desktop_dir/release/mac/ViloForge.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("ViloForge is damaged and can't be opened").
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the ViloForge-managed Node
