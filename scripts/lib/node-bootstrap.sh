...
#   2. ~/.hermes/node/ from a prior ViloForge-managed install
...
# Redirect a ViloForge-managed Node's `npm install -g` to the command link dir
...
ensure_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        export PATH="$HERMES_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (ViloForge-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
