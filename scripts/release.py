#!/usr/bin/env python3
"""ViloForge Agent Release Script
...
AUTHOR_MAP = {
    "jason.viloria@optiscangroup.com": "vilosource",  # ViloForge fork operator (commit author on fork PRs)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local ViloForge runtime files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live ViloForge
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-agent",
    ...
    lines.append(f"# ViloForge Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ViloForge Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ViloForge Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  ViloForge Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ViloForge Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ViloForge Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ViloForge Agent v{new_version} ({calver_date})' "
