...
# Bootstrap Open WebUI against ViloForge Agent's OpenAI-compatible API server.
#
...
#   OPEN_WEBUI_NAME='Johnny ViloForge'
...
#   HERMES_API_MODEL_NAME='ViloForge Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-ViloForge Agent WebUI}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-ViloForge Agent}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to ViloForge Agent
...
main() {
  ...
  log 'Ensuring ViloForge API server is configured...'
  ...
  log 'Restarting ViloForge gateway so API server settings take effect...'
  hermes gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'ViloForge API server did not answer on the first check. Trying to start gateway in the background...'
  ...
  log "ViloForge API endpoint: ${HERMES_API_BASE_URL}"
