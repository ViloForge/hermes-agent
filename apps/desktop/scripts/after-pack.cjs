 ...
 *
 * Stamps the ViloForge icon + identity onto the packed Windows ViloForge.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'ViloForge')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'ViloForge'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); ViloForge.exe keeps the stock Electron icon`)
  }
}
