#!/usr/bin/env node
// set-exe-identity.cjs — stamp the ViloForge icon + version metadata onto the
// built ViloForge.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked ViloForge.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-ViloForge.exe>
//
...
// Stamp the ViloForge icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'ViloForge',
      FileDescription: 'ViloForge',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — ViloForge icon + identity stamped')
}
