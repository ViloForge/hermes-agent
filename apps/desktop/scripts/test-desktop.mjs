...
// no longer bundles the ViloForge Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'ViloForge.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'ViloForge'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'ViloForge.exe'),
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `ViloForge-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `ViloForge-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `ViloForge-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'ViloForge-${version}-${os}-${arch}.${ext}'
...
//   - The ViloForge Agent Python payload is NOT shipped (it's fetched at first
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing ViloForge
