 ...
 *
 *   macOS:   …/ViloForge.app/Contents/MacOS/ViloForge  → …/ViloForge.app
 *   Windows: …\ViloForge\ViloForge.exe                 → …\ViloForge  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/ViloForge.app/Contents/MacOS/ViloForge → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ViloForge.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs ViloForge.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]ViloForge$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) return dir
    return null
  }
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  // a problem, but ViloForge install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
