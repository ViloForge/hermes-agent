...
// HERMES_HOME — the user-facing root for everything ViloForge-related. Mirrors
...
// ACTIVE_HERMES_ROOT — the canonical mutable ViloForge install. Same path
...
// active-profile.json records which ViloForge profile the desktop launches its
...
// Seed the native About panel with the live ViloForge version. This is refreshed
...
let bootProgressState = {
  ...
  message: 'Waiting to start ViloForge backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — ViloForge will start automatically when it completes…',
      12
    )
...
function findSystemPython() {
  ...
  // CPython but several ViloForge deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. ViloForge' terminal tool requires
...
// `hermes update`). Instead we spawn the staged ViloForge-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating ViloForge — this window will close and the updater will open. Don’t reopen ViloForge yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
...
async function applyUpdatesPosixInApp() {
  ...
  // Put the ViloForge-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating ViloForge (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart ViloForge to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting ViloForge…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen ViloForge to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the ViloForge desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen ViloForge to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'ViloForge.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'ViloForge.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart ViloForge to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart ViloForge to load the new version.',
      percent: 100
    })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/ViloForge.app/Contents/...`
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `ViloForge at ${ACTIVE_HERMES_ROOT}`,
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `ViloForge source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `ViloForge source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows ViloForge override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving ViloForge CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return {
          label: `existing ViloForge CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing ViloForge CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'ViloForge Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no ViloForge install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('ViloForge recovery was handed off to ViloForge Setup. The desktop will restart when recovery completes.')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('ViloForge install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `ViloForge bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `ViloForge install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. ViloForge' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for ViloForge on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch ViloForge.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `ViloForge venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `ViloForge at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'ViloForge runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported ViloForge backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the ViloForge backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to ViloForge backend after ${timeoutMs}ms`))
    })
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`ViloForge backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting ViloForge...".
...
//
// Hosted ViloForge gateways gate the dashboard behind an OAuth provider (e.g.
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to ViloForge gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to ViloForge backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote ViloForge gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote ViloForge gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote ViloForge backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every ViloForge
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to ViloForge gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting ViloForge backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`ViloForge backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`ViloForge backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`ViloForge backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `ViloForge backend for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving ViloForge backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote ViloForge backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote ViloForge backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving ViloForge runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(dashboardArgs))
    const hermesCwd = resolveHermesCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting ViloForge backend via ${backend.label}`, 84)
    rememberLog(`Starting ViloForge backend via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      rememberLog(`ViloForge backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `ViloForge backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`ViloForge backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `ViloForge backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `ViloForge backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for ViloForge backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(hermesProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for ViloForge backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'ViloForge backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while ViloForge is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main ViloForge app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the ViloForge icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep ViloForge in the normal
...
// "Starting ViloForge…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('Cached remote ViloForge backend failed liveness probe; dropping stale connection.')
    resetHermesConnection()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip ViloForge' TUI into
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart ViloForge.')
  }
...
// Resolve the canonical ViloForge version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real ViloForge version instead of the Electron app's own package.json version,
...
// Re-resolve the live ViloForge version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no ViloForge agent venv at ${VENV_ROOT}.`
    }
  }
