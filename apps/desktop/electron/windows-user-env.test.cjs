...
test('parseRegQueryValue extracts a REG_SZ value', () => {
  const out = [
    ...
    '    HERMES_HOME    REG_SZ    F:\\ViloForge\\data',
    ''
  ].join('\r\n')
  assert.equal(parseRegQueryValue(out, 'HERMES_HOME'), 'F:\\ViloForge\\data')
})
...
test('parseRegQueryValue preserves spaces inside the value', () => {
  const out = '    HERMES_HOME    REG_SZ    C:\\Program Files\\ViloForge\r\n'
  assert.equal(parseRegQueryValue(out, 'HERMES_HOME'), 'C:\\Program Files\\ViloForge')
})
...
test('expandWindowsEnvRefs leaves literal paths and unknown refs intact', () => {
  assert.equal(expandWindowsEnvRefs('F:\\ViloForge\\data', {}), 'F:\\ViloForge\\data')
...
test('readWindowsUserEnvVar queries HKCU\\Environment and expands the value', () => {
  ...
  const exec = (cmd, args) => {
    calls.push([cmd, args])
    return 'HKEY_CURRENT_USER\\Environment\r\n    HERMES_HOME    REG_EXPAND_SZ    %DRIVE%\\ViloForge\r\n'
  }
  ...
  assert.equal(value, 'F:\\ViloForge')
