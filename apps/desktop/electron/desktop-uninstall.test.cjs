...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/ViloForge.app/Contents/MacOS/ViloForge', 'darwin'),
    '/Applications/ViloForge.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/ViloForge.app/Contents/MacOS/ViloForge', 'darwin'),
    '/Users/x/Applications/ViloForge.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\ViloForge\\ViloForge.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\ViloForge'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\ViloForge.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\ViloForge.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HermesXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/ViloForge.AppImage' }),
    '/home/x/Apps/ViloForge.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/ViloForge.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/ViloForge.app'), false)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\ViloForge',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\ViloForge" >nul 2>&1/)
