...
function ConnectedProviderRow({
  ...
  // ViloForge can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider ViloForge can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // ViloForge never deletes creds another tool owns behind a silent API call.
