...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted ViloForge & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when ViloForge needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before ViloForge stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters ViloForge can read from one file request.',
  approvals: {
    mode: 'How ViloForge handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When ViloForge updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
