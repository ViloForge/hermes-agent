...
describe('connecting overlay vs recovery surface', () => {
  it('hard initial-boot failure surfaces the recovery overlay (the working path)', () => {
    ...
    $desktopBoot.set({ ...$desktopBoot.get(), error: 'ViloForge backend did not become ready', running: false, visible: true })
  ...
  it('FIX: once the prolonged reconnect raises a recoverable boot error, the recovery overlay takes over', () => {
    ...
    $desktopBoot.set({
      ...$desktopBoot.get(),
      error: 'Lost connection to the ViloForge gateway and could not reconnect.',
      running: false,
      visible: true
    })
