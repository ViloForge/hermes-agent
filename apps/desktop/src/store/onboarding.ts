...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'ViloForge is ready', message: `${provider} connected.` })
}
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but ViloForge still cannot resolve a usable provider. ${detail}`
    : 'Connected, but ViloForge still cannot resolve a usable provider.'
}
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `ViloForge still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but ViloForge still cannot reach ${url}.` }
    }
