...
//!
//! Driven when the installer is launched as `ViloForge-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old ViloForge desktop process to fully exit (so both the
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating ViloForge"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let hermes = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the hermes CLI under {}. Is ViloForge installed? \
    ...
    // and the guard would only produce a false "ViloForge is still running" stop.
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close ViloForge" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "ViloForge is still running. Close all ViloForge windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch ViloForge manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for ViloForge to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend hermes.exe (or the desktop ViloForge.exe
            ...
            // force-kill every ViloForge.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] ViloForge still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("ViloForge.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("ViloForge.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no ViloForge.app was found under {}",
...
mod tests {
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/ViloForge.app"]),
            Some(PathBuf::from("/Applications/ViloForge.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("ViloForge.app");
        let tmp = base.join("ViloForge.app.hermes-update-new");
        let old = base.join("ViloForge.app.hermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("ViloForge.app");
        let tmp = base.join("ViloForge.app.hermes-update-new"); // intentionally absent
        let old = base.join("ViloForge.app.hermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("ViloForge.app");
        let tmp = base.join("ViloForge.app.hermes-update-new"); // absent
        let old = base.join("ViloForge.app.hermes-update-old");
        write_marker(&target, "OLD");
