//! ViloForge Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `ViloForge-Setup.exe --update`).
///
...
pub fn run() {
    ...
    // ViloForge is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "ViloForge installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // ViloForge is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "ViloForge" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "ViloForge" shortcuts that install.ps1 creates, and a
        ...
        .expect("error while running ViloForge Setup");
}
