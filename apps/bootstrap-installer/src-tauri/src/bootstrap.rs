...
/// Spawn the locally-built ViloForge desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built ViloForge desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching ViloForge desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/ViloForge
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "ViloForge.exe"),
            ("win-arm64-unpacked", "ViloForge.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/ViloForge.app/Contents/MacOS", "ViloForge"),
            ("mac-arm64/ViloForge.app/Contents/MacOS", "ViloForge"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../ViloForge.app/Contents/MacOS/ViloForge -> .../ViloForge.app
...
/// path so a bare re-open just opens ViloForge instead of re-running setup.
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built ViloForge desktop app")
    })?;
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("ViloForge"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../ViloForge.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("ViloForge.exe");
    ...
    // what the updater ditto's over /Applications/ViloForge.app). A regression in
