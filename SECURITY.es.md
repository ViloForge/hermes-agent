# Política de Seguridad de ViloForge Agent

Este documento describe el modelo de confianza de ViloForge Agent, identifica el
...
vulnerabilidades de seguridad. **ViloForge Agent no opera un programa de
...
ViloForge Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta ViloForge Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a ViloForge Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de ViloForge Agent
...
ViloForge Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
ViloForge Agent admite esto de dos maneras:

- **La propia imagen Docker de ViloForge Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de ViloForge Agent
...
ViloForge Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en ViloForge Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de ViloForge Agent
...
**Superficies en ViloForge Agent:**
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   ViloForge Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de ViloForge Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  ViloForge Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de ViloForge Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en ViloForge Agent. Los errores en la ruta de
  instalación de habilidades o plugins de ViloForge Agent que impidan al
...
- ViloForge Agent incluye guardias de cadena de suministro para lanzamientos de servidores
