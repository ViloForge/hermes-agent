...
Maps each role archetype to the ViloForge skills it should `always_load` and the
...
## ViloForge skills relevant to video production
...
These are **not** ViloForge skills but external CLIs / APIs that profiles invoke.
...
## Built-in ViloForge tools for media review

These are native ViloForge tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every ViloForge profile (Claude) |
...
When a new ViloForge-public video skill ships:
