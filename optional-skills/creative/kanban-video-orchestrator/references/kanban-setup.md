...
the actual `setup.sh` that creates the project workspace, configures ViloForge
...
PyYAML must be installed in the user's Python (it ships with most ViloForge
...
the ViloForge `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
