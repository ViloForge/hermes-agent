...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (ViloForge uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — ViloForge has `image_generate`
...
- Expansion with palettes + video: ViloForge Agent contributors
