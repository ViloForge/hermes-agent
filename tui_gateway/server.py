...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base ViloForge): with no explicit pin, collapse to the
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    ViloForge has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the ViloForge profile home
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not ViloForge Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the ViloForge/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not ViloForge Desktop. If it serves ViloForge/Desktop UI or another unrelated app, stop that process and report failure.",
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No ViloForge provider is configured.",
                },
            )
