"""
ViloForge MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ViloForge-specific extra)
...
class EventBridge:
    ...
    This is the ViloForge equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ViloForge MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "ViloForge Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ViloForge MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
